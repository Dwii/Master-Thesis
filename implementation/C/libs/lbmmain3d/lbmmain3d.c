    ...
    for (size_t x = 0; x < width; x++) {
        for (size_t y = 0; y < height; y++) {
            for (size_t z = 0; z < depth; z++) {
                lbm_lattice lattice;
                lbm_lattices_at_index(&lattice, lattices, x, y, z);
                fprintf(file, "%64.60f\n", lattice.ne);
                fprintf(file, "%64.60f\n", lattice.e );
                fprintf(file, "%64.60f\n", lattice.se);
                fprintf(file, "%64.60f\n", lattice.n );
...
static void output_image(char* filename, size_t width, size_t height, size_t z, lbm_u* u)
{
...
int main(int argc, char * const argv[])
{
    ...
    lbm_simulation_get_size(lbm_sim, &width, &height, &depth);
        ...
        if ( (!out_interval && iter == max_iter) || (out_interval && iter % out_interval == 0) ) {
            ...
            if ( out == OUT_IMG ) {
                ...
                if ( asprintf(&filename, "%s/%s%d.pgm", out_path, out_pref, iter) != -1 ) {
                    output_image(filename, width, height, depth/2, u);
                    free(filename);
                }
            }
