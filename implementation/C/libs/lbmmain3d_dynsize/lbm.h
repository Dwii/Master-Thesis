...
lbm_simulation* lbm_simulation_create(size_t width, size_t height, size_t depth);
void lbm_simulation_destroy(lbm_simulation* lbm_sim);
void lbm_simulation_update(lbm_simulation* lbm_sim);

lbm_lattices* lbm_lattices_create(size_t width, size_t height, size_t depth);
void lbm_lattices_destroy(lbm_lattices* lat);
void lbm_lattices_read(lbm_simulation* lbm_sim, lbm_lattices* lat);
void lbm_lattices_at_index(lbm_lattice* lattice, lbm_lattices* lattices, int x, int y, int z, size_t width, size_t height, size_t depth);
...
void lbm_u_at_index(double* u0, double* u1, double* u2, lbm_u* u, int x, int y, int z, size_t width, size_t height, size_t depth);
