...
struct ELBMOperations {
    static bool getAlpha(T &alpha, Array<T,Descriptor<T>::q> const& f, Array<T,Descriptor<T>::q> const& fNeq)
    {
        ...
        if(dev < 1.e-6){
            ...
            alpha = 2 - 4*a2/a1 + 16*a2*a2/(a1*a1) - 8*a3/a1 + 80*a2*a3/(a1*a1) -
                    80*a2*a2*a2/(a1*a1*a1) - 16*a4/a1;
            return true;
        }
        else{
    ...
    static T computeEntropyGrowthDerivative(Array<T,Descriptor<T>::q> const& f, Array<T,Descriptor<T>::q> const& fNeq, T alpha)
    {
        typedef Descriptor<T> L;
    ...
    static T computeEntropyGrowth(Array<T,Descriptor<T>::q> const& f, Array<T,Descriptor<T>::q> const& fNeq, T alpha)
    {
        typedef Descriptor<T> L;
...
int ForcedEntropicDynamics<T,Descriptor>::id =
    meta::registerGeneralDynamics<T,Descriptor,ForcedEntropicDynamics<T,Descriptor> >("Entropic_Forced");
...
template<typename T, template<typename U> class Descriptor>
ForcedEntropicDynamics<T,Descriptor>::ForcedEntropicDynamics(HierarchicUnserializer& unserializer)
...
template<typename T, template<typename U> class Descriptor>
void EntropicDynamics<T,Descriptor>::collideExternal(Cell<T,Descriptor>& cell, T rhoBar,
...
int VariableOmegaELBMDynamics<T,Descriptor>::id =
    meta::registerGeneralDynamics<T,Descriptor,VariableOmegaELBMDynamics<T,Descriptor> >("VariableOmegaELBMDynamics");

template<typename T, template<typename U> class Descriptor>
VariableOmegaELBMDynamics<T,Descriptor>::VariableOmegaELBMDynamics(
        T omega0_, bool automaticPrepareCollision_)
    : VariableOmegaDynamics<T,Descriptor>(new BGKdynamics<T,Descriptor>(omega0_),
                                          automaticPrepareCollision_),
      omega0(omega0_)
{ }

template<typename T, template<typename U> class Descriptor>
VariableOmegaELBMDynamics<T,Descriptor>::VariableOmegaELBMDynamics(HierarchicUnserializer& unserializer)
    : VariableOmegaDynamics<T,Descriptor>(0, false),
      omega0(T())
{
