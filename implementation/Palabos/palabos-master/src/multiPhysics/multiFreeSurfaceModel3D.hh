...
void MultiFreeSurfaceOneWayCoupling3D<T,Descriptor>
        ...
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {         
                if (param1.flag(iX,iY,iZ)==interface && isWet(param2.flag(iX,iY,iZ))) {
                    ...
                    T rhoBar1; 
                    Array<T,3> j1;
                    momentTemplates<T,Descriptor>::get_rhoBar_j(param1.cell(iX,iY,iZ), rhoBar1, j1);
...
template<typename T,template<typename U> class Descriptor>
void MultiFreeSurfaceVelocityContinuityCoupling3D<T,Descriptor>
        ::processGenericBlocks(Box3D domain,std::vector<AtomicBlock3D*> atomicBlocks)
{
    PLB_ASSERT(atomicBlocks.size() == 20);
                        ...
                        for (plint j=-1; j<2; j++) {
                            for (plint k=-1; k<2; k++) {
                                ...
                                if (isFullWet(param2.flag(iX+i,iY+j,iZ+k))) {
                                    ++numNeighbors;
                                    averageDensity += convertRho2to1 * param2.getDensity(iX+i,iY+j,iZ+k);
                                }
                            }
                        }
                    }
                    ...
                    for (plint i=-1; i<2; i++) {
                        for (plint j=-1; j<2; j++) {
                            for (plint k=-1; k<2; k++) {
                                if (isFullWet(param1.flag(iX+i,iY+j,iZ+k))) {
                                    ++numNeighbors;
                                    averageDensity += convertRho1to2 * param1.getDensity(iX+i,iY+j,iZ+k);
                                }
                                if (isFullWet(param2.flag(iX+i,iY+j,iZ+k))) {
                                    ++numNeighbors;
...
template<typename T,template<typename U> class Descriptor>
void MultiFreeSurfaceRepellingForceCoupling3D<T,Descriptor>
        ::processGenericBlocks(Box3D domain,std::vector<AtomicBlock3D*> atomicBlocks)
{
    PLB_ASSERT(atomicBlocks.size() == 20);
        ...
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {         
                        ...
                        for (plint j=-2; j<3; j++) {
                            ...
                            for (plint k=-2; k<3; k++) {
                                ...
                                if (isWet(param1.flag(I,J,K)) && isWet(param2.flag(I,J,K))) {
                                    T VFdiff = param1.volumeFraction(I,J,K) + param2.volumeFraction(I,J,K) - 1.0;
                                    if (VFdiff>(T)0.0) {
                ...
                if (param1.flag(iX,iY,iZ)==interface && isWet(param2.flag(iX,iY,iZ))) {
                    ...
                    // density differences. In order to have the units of a momentum, this correction term must be
                    // multiplied by the density.
                    T VFdiff = param1.volumeFraction(iX,iY,iZ) + param2.volumeFraction(iX,iY,iZ) - 1.0;
...
template<typename T,template<typename U> class Descriptor>
void MultiFreeSurfaceComplexCoupling3D<T,Descriptor>
        ::processGenericBlocks(Box3D domain,std::vector<AtomicBlock3D*> atomicBlocks)
{
    PLB_ASSERT(atomicBlocks.size() == 20);
