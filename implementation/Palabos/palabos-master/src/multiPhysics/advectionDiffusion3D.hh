...
template< typename T, template<typename U> class TemperatureDescriptor >
void velocityToPassiveAdvDiff(MultiBlockLattice3D<T,TemperatureDescriptor>& temperature, MultiTensorField3D<T,3>& velocity, Box3D domain)
{
        ...
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                *scalar.get(iX+offset.x,iY+offset.y,iZ+offset.z).getExternal(rhoBarOffset) = rhoBar;
                ...
                piNeq.to_cArray(scalar.get(iX+offset.x,iY+offset.y,iZ+offset.z).getExternal(piNeqOffset));
            }
        }
    }
}
...
{
    applyProcessingFunctional (
            new LatticeToPassiveComplRegAdvDiff3D<T,FluidDescriptor,ScalarDescriptor>(), domain, fluid, scalar);
}
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                Cell<T,ScalarDescriptor>& scalarCell = scalar.get(iX+offset.x,iY+offset.y,iZ+offset.z);
                ...
                T fluidOmega = fluidCell.getDynamics().getDynamicParameter(dynamicParams::dynamicOmega, fluidCell);
                if (!util::isZero(fluidOmega)) {
                    T fluidOmega0 = fluidCell.getDynamics().getOmega();
                    T scalarOmega0 = scalarCell.getDynamics().getOmega();
                    T scalarOmega = (T) 1 / ((T) 1 / scalarOmega0 + C * ((T) 1 / fluidOmega - (T) 1 / fluidOmega0));
...
void turbulentLatticeToPassiveAdvDiff (
        MultiBlockLattice3D<T,FluidDescriptor>& fluid, MultiBlockLattice3D<T,ScalarDescriptor>& scalar, T Pr_t, Box3D domain )
{
    applyProcessingFunctional (
            new TurbulentLatticeToPassiveAdvDiff3D<T,FluidDescriptor,ScalarDescriptor>(Pr_t), domain, fluid, scalar);
}
              ...
              Box3D domain, std::vector<AtomicBlock3D*> blocks )
{
    ...
    BlockLattice3D<T,FluidDescriptor>* fluid = dynamic_cast<BlockLattice3D<T,FluidDescriptor>*>(blocks[0]);
    BlockLattice3D<T,ScalarDescriptor>* scalar = dynamic_cast<BlockLattice3D<T,ScalarDescriptor>*>(blocks[1]);
        ...
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                if (mask->get(iX+ofsMask.x,iY+ofsMask.y,iZ+ofsMask.z) == whichFlag) {
                    Cell<T,FluidDescriptor>& fluidCell = fluid->get(iX,iY,iZ);
                    Cell<T,ScalarDescriptor>& scalarCell = scalar->get(iX+ofsScalar.x,iY+ofsScalar.y,iZ+ofsScalar.z);
...
{
    return new MaskedTurbulentLatticeToPassiveAdvDiff3D<T,FluidDescriptor,ScalarDescriptor>(*this);
}
...
void maskedTurbulentLatticeToPassiveAdvDiff (
        MultiBlockLattice3D<T,FluidDescriptor>& fluid, MultiBlockLattice3D<T,ScalarDescriptor>& scalar,
        MultiScalarField3D<int>& mask, T Pr_t, int whichFlag, Box3D domain )
{
    ...
    args.push_back(&mask);
    applyProcessingFunctional (
            new MaskedTurbulentLatticeToPassiveAdvDiff3D<T,FluidDescriptor,ScalarDescriptor>(Pr_t, whichFlag), domain, args);
}
...
{
    return new N_MaskedTurbulentLatticeToPassiveAdvDiff3D<T,FluidDescriptor,ScalarDescriptor>(*this);
}
...
void NMaskedTurbulentLatticeToPassiveAdvDiff (
        MultiBlockLattice3D<T,FluidDescriptor>& fluid, MultiBlockLattice3D<T,ScalarDescriptor>& scalar,
        MultiNTensorField3D<int>& mask, T Pr_t, int whichFlag, Box3D domain )
{
    std::vector<MultiBlock3D*> args;
    args.push_back(&fluid);
    args.push_back(&scalar);
    args.push_back(&mask);
    applyProcessingFunctional (
            new N_MaskedTurbulentLatticeToPassiveAdvDiff3D<T,FluidDescriptor,ScalarDescriptor>(Pr_t, whichFlag), domain, args);
}
        ...
        Box3D domain, std::vector<AtomicBlock3D*> blocks)
{
    ...
    PLB_ASSERT(blocks.size() == 3);
    BlockLattice3D<T,AD_Descriptor>* tempLattice = dynamic_cast<BlockLattice3D<T,AD_Descriptor>*>(blocks[0]);
    BlockLattice3D<T,FS_Descriptor>* fluidLattice = dynamic_cast<BlockLattice3D<T,FS_Descriptor>*>(blocks[1]);
    ...
    Array<T,3> zero; zero.resetToZero();
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                    ...
                    for (plint dx = -d; dx <= d; dx++) {
                        ...
                        for (plint dy = -d; dy <= d; dy++) {
                            ...
                            for (plint dz = -d; dz <= d; dz++) {
                                ...
                                //if(!(dx==0 && dy==0 && dz==0) && (nextCell.getDynamics().getId()==conductiveId) &&
                                //        (nextFlag!=wall)) {
                                if(!(dx==0 && dy==0 && dz==0) && (nextCell.getDynamics().getId()==conductiveId)) {
                                    for (plint iPop=0; iPop<AD_Descriptor<T>::q; ++iPop) {
                ...
                Cell<T,FS_Descriptor>& fluidCell = fluidLattice->get(iX+ofsFluid.x,iY+ofsFluid.y,iZ+ofsFluid.z);
                ...
                if (tempCell.getDynamics().getId() != adiabaticId) {
                    ...
                    if (!util::isZero(fluidOmega)) {
                        ...
                        T tempOmega = (T) 1 / ((T) 1 / tempOmega0 + C * ((T) 1 / fluidOmega - (T) 1 / fluidOmega0));
...
MaskedAdvectionDiffusionFreeSurfaceCoupling3D(
        ...
        T iniVal_, T Pr_t, int maskValue_)
    : conductiveDynamics(conductiveDynamics_),
      adiabaticDynamics(adiabaticDynamics_),
      iniVal(iniVal_),
...
template<typename T>
AdvectionDiffusionFd3D<T>::AdvectionDiffusionFd3D(T d_, bool upwind_)
    : d(d_),
      upwind(upwind_)
{ }
        ...
        for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
            for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
                for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                    ...
                    T advection = u[0] * (util::greaterThan(u[0], (T) 0) ? (phiC - phiW) :
                                         (util::lessThan(   u[0], (T) 0) ? (phiE - phiC) : (T) 0.5 * (phiE - phiW))) +
                                  u[1] * (util::greaterThan(u[1], (T) 0) ? (phiC - phiS) :
                                         (util::lessThan(   u[1], (T) 0) ? (phiN - phiC) : (T) 0.5 * (phiN - phiS))) +
                                  u[2] * (util::greaterThan(u[2], (T) 0) ? (phiC - phiB) :
                                         (util::lessThan(   u[2], (T) 0) ? (phiT - phiC) : (T) 0.5 * (phiT - phiB)));
        ...
        for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
            for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
                for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                    ...
                    Array<T,3> const& u = uField->get(iX+ofs3.x, iY+ofs3.y, iZ+ofs3.z);
                    T advection = (T) 0.5 * (u[0] * (phiE - phiW) + u[1] * (phiN - phiS) + u[2] * (phiT - phiB));
...
template<typename T>
VariableDiffusivityAdvectionDiffusionFd3D<T>::VariableDiffusivityAdvectionDiffusionFd3D(bool upwind_)
    : upwind(upwind_)
{ }

template<typename T>
void VariableDiffusivityAdvectionDiffusionFd3D<T>::processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> fields)
{
        ...
        for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
            for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
                for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                    ...
                    T diffusion = DE * (phiE - phiC) - DW * (phiC - phiW) +
                                  DN * (phiN - phiC) - DS * (phiC - phiS) +
                                  DT * (phiT - phiC) - DB * (phiC - phiB);
...
template<typename T>
VariableDiffusivityAdvectionDiffusionFd3D<T>* VariableDiffusivityAdvectionDiffusionFd3D<T>::clone() const
{
    return new VariableDiffusivityAdvectionDiffusionFd3D<T>(*this);
}

template<typename T>
void VariableDiffusivityAdvectionDiffusionFd3D<T>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;           // phi_t
    modified[1] = modif::nothing;           // phi_tp1
    modified[2] = modif::staticVariables;   // result
    modified[3] = modif::nothing;           // u
...
MaskedVariableDiffusivityAdvectionDiffusionFd3D<T,ADCellT>::MaskedVariableDiffusivityAdvectionDiffusionFd3D(
        ADCellT adCellT_, T defaultValue_, Box3D const& fullClosedDomain_, bool xPeriodic_, bool yPeriodic_, bool zPeriodic_,
        bool upwind_)
    : adCellT(adCellT_),
...
template<typename T, class ADCellT>
void MaskedVariableDiffusivityAdvectionDiffusionFd3D<T,ADCellT>::processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> fields)
{
                ...
                if (adCellT.isSingular(flagC)) {
                    ...
                    for (plint dx=-1; dx<=1; ++dx) {
                        ...
                        for (plint dy=-1; dy<=1; ++dy) {
                            ...
                            for (plint dz=-1; dz<=1; ++dz) {
                                plint nextZ = iZ + dz;
                                if (contained(nextX + absOfs.x, nextY + absOfs.y, nextZ + absOfs.z, fullExtendedDomain)) {
                                    int nextFlag = flag->get(nextX+ofs6.x, nextY+ofs6.y, nextZ+ofs6.z);
                                    if (!adCellT.isSingular( nextFlag) &&
                                        !adCellT.isZeroFlux( nextFlag) &&
                                        !adCellT.isFixedFlux(nextFlag, phi, dphi)) {
                                        avePhi += phi_tp1->get(nextX+ofs1.x, nextY+ofs1.y, nextZ+ofs1.z);
                                        n++;
                                    }
                                }
                            }
                            ...
                            for (plint dz=-1; dz<=1; ++dz) {
                                ...
                                if (contained(nextX + absOfs.x, nextY + absOfs.y, nextZ + absOfs.z, fullExtendedDomain)) {
                                    ...
                                    if (!adCellT.isSingular( nextFlag) &&
                                        !adCellT.isZeroFlux( nextFlag) &&
                                        !adCellT.isFixedFlux(nextFlag, phi, dphi)) {
                                        phi = phi_tp1->get(nextX+ofs1.x, nextY+ofs1.y, nextZ+ofs1.z);
                                        dphi = (T) 0;
                    ...
                    T phiC = phi_tp1->get(iX+ofs1.x, iY+ofs1.y, iZ+ofs1.z); phi = dphi = (T) 0;

                    T phiE = adCellT.isFixed(    flagE, phi) ? phi :
                            (adCellT.isSingular( flagE) ||
                             adCellT.isZeroFlux( flagE) ||
                             adCellT.isFixedFlux(flagE, phiC, dphi) ? phiC + dphi :
                             phi_tp1->get(iX+1+ofs1.x, iY  +ofs1.y, iZ  +ofs1.z)); phi = dphi = (T) 0;
                    T phiW = adCellT.isFixed(    flagW, phi) ? phi :
                            (adCellT.isSingular( flagW) ||
                             adCellT.isZeroFlux( flagW) ||
                             adCellT.isFixedFlux(flagW, phiC, dphi) ? phiC + dphi :
                             phi_tp1->get(iX-1+ofs1.x, iY  +ofs1.y, iZ  +ofs1.z)); phi = dphi = (T) 0;
                    T phiN = adCellT.isFixed(    flagN, phi) ? phi :
                            (adCellT.isSingular( flagN) ||
                             adCellT.isZeroFlux( flagN) ||
                             adCellT.isFixedFlux(flagN, phiC, dphi) ? phiC + dphi :
                             phi_tp1->get(iX  +ofs1.x, iY+1+ofs1.y, iZ  +ofs1.z)); phi = dphi = (T) 0;
                    T phiS = adCellT.isFixed(    flagS, phi) ? phi :
                            (adCellT.isSingular( flagS) ||
                             adCellT.isZeroFlux( flagS) ||
                             adCellT.isFixedFlux(flagS, phiC, dphi) ? phiC + dphi :
                             phi_tp1->get(iX  +ofs1.x, iY-1+ofs1.y, iZ  +ofs1.z)); phi = dphi = (T) 0;
                    T phiT = adCellT.isFixed(    flagT, phi) ? phi :
                            (adCellT.isSingular( flagT) ||
                             adCellT.isZeroFlux( flagT) ||
                             adCellT.isFixedFlux(flagT, phiC, dphi) ? phiC + dphi :
                             phi_tp1->get(iX  +ofs1.x, iY  +ofs1.y, iZ+1+ofs1.z)); phi = dphi = (T) 0;
                    T phiB = adCellT.isFixed(    flagB, phi) ? phi :
                            (adCellT.isSingular( flagB) ||
                             adCellT.isZeroFlux( flagB) ||
                             adCellT.isFixedFlux(flagB, phiC, dphi) ? phiC + dphi :
                             phi_tp1->get(iX  +ofs1.x, iY  +ofs1.y, iZ-1+ofs1.z)); phi = dphi = (T) 0;
                    ...
                    if (upwind) {
                        advection =
                            u[0] * (util::greaterThan(u[0], (T) 0) ? (phiC - phiW) :
                                   (util::lessThan(   u[0], (T) 0) ? (phiE - phiC) : (T) 0.5 * (phiE - phiW))) +
                            u[1] * (util::greaterThan(u[1], (T) 0) ? (phiC - phiS) :
                                   (util::lessThan(   u[1], (T) 0) ? (phiN - phiC) : (T) 0.5 * (phiN - phiS))) +
                            u[2] * (util::greaterThan(u[2], (T) 0) ? (phiC - phiB) :
                        ...
                        advection = (T) 0.5 * (u[0] * (phiE - phiW) + u[1] * (phiN - phiS) + u[2] * (phiT - phiB));
                    }
...
template<typename T, class ADCellT>
MaskedVariableDiffusivityAdvectionDiffusionFd3D<T,ADCellT>* MaskedVariableDiffusivityAdvectionDiffusionFd3D<T,ADCellT>::clone() const
{
    return new MaskedVariableDiffusivityAdvectionDiffusionFd3D<T,ADCellT>(*this);
}

template<typename T, class ADCellT>
void MaskedVariableDiffusivityAdvectionDiffusionFd3D<T,ADCellT>::getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
    modified[0] = modif::nothing;           // phi_t
    modified[1] = modif::nothing;           // phi_tp1
    modified[2] = modif::staticVariables;   // result
...
template<typename T, template<typename U> class FluidDescriptor>
void computeTurbulentDiffusivity(T D0, T Pr_t, MultiBlockLattice3D<T,FluidDescriptor>& fluid,
        MultiScalarField3D<T>& diffusivity, Box3D domain)
{
    applyProcessingFunctional(new TurbulentDiffusivityFunctional3D<T,FluidDescriptor>(D0, Pr_t), domain, fluid, diffusivity);
}

template<typename T, template<typename U> class FluidDescriptor>
std::auto_ptr<MultiScalarField3D<T> > computeTurbulentDiffusivity(T D0, T Pr_t, MultiBlockLattice3D<T,FluidDescriptor>& fluid,
        Box3D domain)
{
    std::auto_ptr<MultiScalarField3D<T> > diffusivity = generateMultiScalarField<T>(fluid, domain);
...
template<typename T, template<typename U> class FluidDescriptor>
void computeTurbulentDiffusivityFromFlag(std::vector<T> const& D0, T Pr_t, MultiBlockLattice3D<T,FluidDescriptor>& fluid,
        MultiScalarField3D<int>& flags, MultiScalarField3D<T>& diffusivity, Box3D domain)
{
    ...
    applyProcessingFunctional(new TurbulentDiffusivityFromFlagFunctional3D<T,FluidDescriptor>(D0, Pr_t), domain, args);
}

template<typename T, template<typename U> class FluidDescriptor>
std::auto_ptr<MultiScalarField3D<T> > computeTurbulentDiffusivityFromFlag(std::vector<T> const& D0, T Pr_t,
        MultiBlockLattice3D<T,FluidDescriptor>& fluid, MultiScalarField3D<int>& flags, Box3D domain)
{
    std::auto_ptr<MultiScalarField3D<T> > diffusivity = generateMultiScalarField<T>(fluid, domain);
...
template<typename T, template<typename U> class FluidDescriptor>
void computeTurbulentDiffusivityFromScalar(T Pr_t, MultiBlockLattice3D<T,FluidDescriptor>& fluid,
        MultiScalarField3D<T>& D0, MultiScalarField3D<T>& diffusivity, Box3D domain)
{
    std::vector<MultiBlock3D*> args;
    args.push_back(&fluid);
    args.push_back(&D0);
    args.push_back(&diffusivity);
    applyProcessingFunctional(new TurbulentDiffusivityFromScalarFunctional3D<T,FluidDescriptor>(Pr_t), domain, args);
}

template<typename T, template<typename U> class FluidDescriptor>
std::auto_ptr<MultiScalarField3D<T> > computeTurbulentDiffusivityFromScalar(T Pr_t,
        MultiBlockLattice3D<T,FluidDescriptor>& fluid, MultiScalarField3D<T>& D0, Box3D domain)
{
    std::auto_ptr<MultiScalarField3D<T> > diffusivity = generateMultiScalarField<T>(fluid, domain);
...
template<typename T, template<typename U> class FluidDescriptor>
std::auto_ptr<MultiScalarField3D<T> > computeTurbulentDiffusivityFromScalar(T Pr_t,
        MultiBlockLattice3D<T,FluidDescriptor>& fluid, MultiScalarField3D<T>& D0)
{
...
template<typename T>
void ArrheniusChemicalReactionCoupling3D<T>::processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> blocks)
{
        ...
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                for (plint iReaction = 0; iReaction < nReactions; iReaction++) {
                    r[iReaction] = preExponentialFactors[iReaction] *
                        std::exp(-activationEnergies[iReaction] /
                                (R * theta->get(iX + ofsTheta.x, iY + ofsTheta.y, iZ + ofsTheta.z)));
                    for (plint iSpecies = 0; iSpecies < nSpecies; iSpecies++) {
                        r[iReaction] *= std::pow(
                                C[iSpecies]->get(iX + ofsC[iSpecies].x, iY + ofsC[iSpecies].y, iZ + ofsC[iSpecies].z),
                ...
                for (plint iSpecies = 0; iSpecies < nSpecies; iSpecies++) {
                    ...
                    massQ[iSpecies]->get(iX + ofsMQ[iSpecies].x, iY + ofsMQ[iSpecies].y, iZ + ofsMQ[iSpecies].z) = CQ * dt;
                }
            }
        }
    }
...
template<typename T>
void ArrheniusChemicalReactionCoupling3D<T>::getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
