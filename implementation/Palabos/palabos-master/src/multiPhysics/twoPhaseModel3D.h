...
template<typename T, int nDim>
void IniFilteredDensity3D<T,nDim>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
}

template<typename T, int nDim>
void iniFilteredDensity(MultiTensorField3D<T,nDim>& densities, MultiScalarField3D<T>& density, Box3D domain)
{
    applyProcessingFunctional (
            new IniFilteredDensity3D<T,nDim>(), domain, density, densities );
}


template<typename T, int nDim>
class AddFilteredDensity3D :
    public BoxProcessingFunctional3D_ST<T,T,nDim>
{
public:
    virtual void process(Box3D domain, ScalarField3D<T>& scalarField,
...
template<typename T, int nDim>
void AddFilteredDensity3D<T,nDim>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
}

template<typename T, int nDim>
void addFilteredDensity(MultiTensorField3D<T,nDim>& densities, MultiScalarField3D<T>& density, Box3D domain)
{
    applyProcessingFunctional (
            new AddFilteredDensity3D<T,nDim>(), domain, density, densities );
}


template<typename T, int nDim>
class GetFilteredDensity3D :
    public BoxProcessingFunctional3D_ST<T,T,nDim>
{
public:
    virtual void process(Box3D domain, ScalarField3D<T>& scalarField,
...
template<typename T, int nDim>
void GetFilteredDensity3D<T,nDim>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::staticVariables;
    modified[1] = modif::nothing;
}

template<typename T, int nDim>
void getFilteredDensity(MultiTensorField3D<T,nDim>& densities, MultiScalarField3D<T>& density, Box3D domain)
{
    applyProcessingFunctional (
...
public:
    TwoPhaseComputePressure3D(T densityRatio_, T rhoDefault_, TwoPhaseModel model_)
        : densityRatio(densityRatio_),
          ...
          computeFluid2(true)
    { }
    TwoPhaseComputePressure3D(T densityRatio_, T rhoDefault_, TwoPhaseModel model_, bool computeFluid1_, bool computeFluid2_)
        : densityRatio(densityRatio_),
          rhoDefault(rhoDefault_),
          model(model_),
...
public:
    ...
    { }
    TwoPhaseComputeVelocity3D(T densityRatio_, bool computeFluid1_, bool computeFluid2_, bool useFreeSurfaceLimit_)
        : densityRatio(densityRatio_),
          computeFluid1(computeFluid1_),
          computeFluid2(computeFluid2_),
          useFreeSurfaceLimit(useFreeSurfaceLimit_)
...
public:
    TwoPhasePunchSphere3D (
            Array<T,3> const& center_, T radius_, T rhoEmpty_, T referenceDensity_,
...
public:
    TwoPhasePunchRectangle3D (
            Box3D rectangle_, T rhoEmpty_, T referenceDensity_,
            T densityRatio_, Dynamics<T,Descriptor> *dynamicsTemplate2_, TwoPhaseModel model_)
    ...
    TwoPhasePunchRectangle3D (TwoPhasePunchRectangle3D<T,Descriptor> const& rhs)
        : rectangle(rhs.rectangle),
          ...
          model(rhs.model)
    { }
    TwoPhasePunchRectangle3D<T,Descriptor>& operator=(TwoPhasePunchRectangle3D<T,Descriptor> const& rhs)
    {
...
public:
    ...
    Cell<T,Descriptor>& cell2(plint iX, plint iY, plint iZ) {
        ...
        return fluid2_->get(iX+relativeOffsetFluid2.x,iY+relativeOffsetFluid2.y,iZ+relativeOffsetFluid2.z);
    }
    ...
    T& mass2(plint iX, plint iY, plint iZ) {
        PLB_ASSERT(!useFreeSurfaceLimit);
        return mass2_->get(iX+relativeOffsetMass2.x,iY+relativeOffsetMass2.y,iZ+relativeOffsetMass2.z);
    }
    ...
    void setMomentum2(plint iX, plint iY, plint iZ, Array<T,3> const& momentum) {
        PLB_ASSERT(!useFreeSurfaceLimit);
        j2_->get(iX+relativeOffsetJ2.x,iY+relativeOffsetJ2.y,iZ+relativeOffsetJ2.z) = momentum;
    }
    ...
    T getDensity2(plint iX, plint iY, plint iZ) {
        PLB_ASSERT(!useFreeSurfaceLimit);
        return Descriptor<T>::fullRho (
                    rhoBar2_->get(iX+relativeOffsetRhoBar2.x, iY+relativeOffsetRhoBar2.y, iZ+relativeOffsetRhoBar2.z) );
    }
    ...
    std::map<Node,ExtrapolInfo>& emptyToInterface() { PLB_ASSERT(interfaceLists_); return interfaceLists_ -> emptyToInterface; }
    std::map<Node,ExtrapolInfo>& fluidToInterface() { PLB_ASSERT(interfaceLists_); return interfaceLists_ -> fluidToInterface; }
...
private:
    ...
    Dot3D absoluteOffset, relativeOffsetFluid2, relativeOffsetRhoBar,
...
// CAUTION: This data processor must be called only after proper initialization of the TwoPhaseFields3D data structure.
...
public:
    ConstantIniVelocityTwoPhase3D(Array<T,3> velocity_, Function f_, bool imposeToFluid1_, bool imposeToFluid2_, bool useFreeSurfaceLimit_)
        : velocity(velocity_),
          f(f_),
...
public:
    TwoPhaseMassChange3D(bool useFreeSurfaceLimit_)
        : useFreeSurfaceLimit(useFreeSurfaceLimit_)
    { }
    virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> atomicBlocks);
...
public:
    TwoPhaseCompletion3D(bool useFreeSurfaceLimit_)
        : useFreeSurfaceLimit(useFreeSurfaceLimit_)
    { }
...
public:
    TwoPhaseMacroscopic3D( T rhoDefault_, T densityRatio_, T surfaceTension_,
...
void twoPhaseInitializeInterfaceLists3D(MultiContainerBlock3D& interfaceListBlock) {
    ...
    applyProcessingFunctional (
            new TwoPhaseInitializeInterfaceLists3D<T,Descriptor>,
...
public:
    TwoPhaseComputeInterfaceLists3D(TwoPhaseModel model_, T rhoDefault_, T densityRatio_, T surfaceTension_)
        : model(model_),
          rhoDefault(rhoDefault_),
          densityRatio(densityRatio_),
          surfaceTension(surfaceTension_)
...
public:
    TwoPhaseEqualMassExcessReDistribution3D(TwoPhaseModel model_)
        : model(model_)
    { }
    virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> atomicBlocks);
...
struct TwoPhaseFields3D {
    ...
    std::auto_ptr<MultiScalarField3D<T> > computePressure(Box3D domain, bool computeFluid1, bool computeFluid2)
    {
        ...
        applyProcessingFunctional (
                new TwoPhaseComputePressure3D<T,Descriptor>(densityRatio, rhoDefault, model, computeFluid1, computeFluid2),
                domain, args );
        return pressure;
    }
    ...
    std::auto_ptr<MultiTensorField3D<T,3> > computeVelocity(Box3D domain, bool computeFluid1, bool computeFluid2)
    {
        ...
        applyProcessingFunctional (
                new TwoPhaseComputeVelocity3D<T,Descriptor>(densityRatio, computeFluid1, computeFluid2, model==freeSurface),
                domain, args );
        return velocity;
    }
    ...
    void freeSurfaceDataProcessors()
    {
       ...
       //         lattice.getBoundingBox(), twoPhaseArgs, pl );
        ...
        integrateProcessingFunctional (
            new TwoPhaseComputeInterfaceLists3D<T,Descriptor>(model, rhoDefault, densityRatio, surfaceTension),
            lattice.getBoundingBox(), twoPhaseArgs, pl );
