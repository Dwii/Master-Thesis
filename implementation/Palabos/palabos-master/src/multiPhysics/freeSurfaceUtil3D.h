...
namespace freeSurfaceFlag {
    enum Flag {empty=0, interface=1, fluid=2, wall=4, protect=5, temporarilyProtect=6, protectEmpty=7, slipWall=8};
    inline std::string flagToString(int flag) {
        switch(flag) {
            ...
            case temporarilyProtect:    return "temporarilyProtect";
            case protectEmpty:          return "protectEmpty";
            case slipWall:              return "slipWall";
...
public:
    ...
    FreeSurfaceProcessorParam3D(std::vector<AtomicBlock3D*>& atomicBlocks)
    {
        ...
        //PLB_ASSERT(interfaceLists_); // This assertion must be commented out to work with TwoPhase.
    ...
    T smooth(ScalarField3D<T> const& scalar, Dot3D const& ofs, plint iX, plint iY, plint iZ)
    {
        ...
        if (isAnyWall(flag_->get(iX + relativeOffsetFS.x, iY + relativeOffsetFS.y, iZ + relativeOffsetFS.z))) {
        ...
        for (int i = -1; i < 2; i++) {
            ...
            for (int j = -1; j < 2; j++) {
                ...
                for (int k = -1; k < 2; k++) {
                    ...
                    if (!(i == 0 && j == 0 && k == 0) &&
                            !isAnyWall(
                                flag_->get(nextX + relativeOffsetFS.x, nextY + relativeOffsetFS.y, nextZ + relativeOffsetFS.z))) {
                        n++;
    ...
    // The following function can be called with a lattice descriptor "Descriptor2", which is different from the lattice
    // descriptor "Descriptor" used in the simulation.
    ...
    T lbmSmooth(ScalarField3D<T> const& scalar, Dot3D const& ofs, plint iX, plint iY, plint iZ)
    {
        ...
        for (plint iPop = 1; iPop < D::q; iPop++) {
            ...
            if (isAnyWall(flag_->get(nextX + relativeOffsetFS.x, nextY + relativeOffsetFS.y, nextZ + relativeOffsetFS.z))) {
                ...
                if (isAnyWall(flag_->get(prevX + relativeOffsetFS.x, prevY + relativeOffsetFS.y, prevZ + relativeOffsetFS.z))) {
                    nextVal = locVal;
                } else {
    ...
    // descriptor "Descriptor" used in the simulation.
    template<typename T2, template<typename U2> class Descriptor2>
    Array<T,3> lbmSmooth(TensorField3D<T,3> const& tensor, Dot3D const& ofs, plint iX, plint iY, plint iZ)
    {
        typedef Descriptor2<T2> D;
        using namespace freeSurfaceFlag;
    ...
    // In the free-surface algorithm there is the need to compute derivatives with finite differences.
    // The following function, computes the x/y/z widths and positions for finite difference stencils
    // depending on the local point in the simulation domain. The stencils are tuned so that lattice nodes
    // inside walls are excluded. The maximum stencil width is always an odd number of the form:
    // 2 * h + 1, with h being less or equal to the envelope width of the flag matrix.
    void getFdStencilWidthsAndPositions(plint iX, plint iY, plint iZ, plint h,
            Array<int,3>& widths, Array<int,3>& positions)
    {
        using namespace freeSurfaceFlag;
    ...
    Array<T,3> computeGradient(ScalarField3D<T> const& scalar, Dot3D const& ofs, plint h, plint iX, plint iY, plint iZ)
    {
        using namespace freeSurfaceFlag;

        //PLB_ASSERT(h >= 0 && h <= 3);
        //PLB_ASSERT(!isAnyWall(flag_->get(iX + relativeOffsetFS.x, iY + relativeOffsetFS.y, iZ + relativeOffsetFS.z)));
        ...
        // 7: this is at least (2 * h + 1), with "h" being at most equal to the envelope width of the flag matrix
        //    We set this to its current possible maximum value 7, to be more efficient in case we want to use
        //    high-order finite differences. This must be changed if "h" is ever expected to be larger than 3.
    ...
    T computeDivergence(TensorField3D<T,3> const& tensor, Dot3D const& ofs, plint h, plint iX, plint iY, plint iZ)
    {
        using namespace freeSurfaceFlag;
        ...
        //    We set this to its current possible maximum value 7, to be more efficient in case we want to use
        //    high-order finite differences. This must be changed if "h" is ever expected to be larger than 3.
        Array<T,3> d_dx = computeTensorXderivative<T, 3, 1, 7>(tensor, widths[0], positions[0], i, j, k);
        Array<T,3> d_dy = computeTensorYderivative<T, 3, 1, 7>(tensor, widths[1], positions[1], i, j, k);
        Array<T,3> d_dz = computeTensorZderivative<T, 3, 1, 7>(tensor, widths[2], positions[2], i, j, k);
    ...
    // With this method the scalar field values are first extrapolated on the neighboring wall cells (if necessary).
    // So, in contrast to the above finite difference implementation, the wall cells are not excluded here.
    // The following function can be called with a lattice descriptor "Descriptor2", which is different from the lattice
    // descriptor "Descriptor" used in the simulation.
    template<typename T2, template<typename U2> class Descriptor2>
    Array<T,3> lbmComputeGradient(ScalarField3D<T> const& scalar, Dot3D const& ofs, plint iX, plint iY, plint iZ)
    {
        typedef Descriptor2<T2> D;
        using namespace freeSurfaceFlag;
    ...
    // With this method the tensor field values are first extrapolated on the neighboring wall cells (if necessary).
    // So, in contrast to the above finite difference implementation, the wall cells are not excluded here.
    // The following function can be called with a lattice descriptor "Descriptor2", which is different from the lattice
    // descriptor "Descriptor" used in the simulation.
    template<typename T2, template<typename U2> class Descriptor2>
    T lbmComputeDivergence(TensorField3D<T,3> const& tensor, Dot3D const& ofs, plint iX, plint iY, plint iZ)
    {
        typedef Descriptor2<T2> D;
        using namespace freeSurfaceFlag;
        ...
        for (plint iPop = 1; iPop < D::q; ++iPop ) {
            ...
            divergence += D::t[iPop] * (D::c[iPop][0] * nextVal[0] + D::c[iPop][1] * nextVal[1] + D::c[iPop][2] * nextVal[2]);
        }
        divergence *= D::invCs2;
    ...
    std::set<Node>& interfaceToFluid() { return interfaceLists_ -> interfaceToFluid; }
    std::set<Node>& interfaceToEmpty() { return interfaceLists_ -> interfaceToEmpty; }
    std::set<Node>& emptyToInterface() { return interfaceLists_ -> emptyToInterface; }

    BlockLattice3D<T,Descriptor>* latticeP() { return fluid_; }
    ScalarField3D<T>* rhoBarP() { return rhoBar_; }
    TensorField3D<T,3>* jP() { return j_; }
    ScalarField3D<T>* massP() { return mass_; }
    ScalarField3D<T>* volumeFractionP() { return volumeFraction_; }
    ScalarField3D<int>* flagP() { return flag_; }
    TensorField3D<T,3>* normalP() { return normal_; }
    ScalarField3D<T>* curvatureP() { return curvature_; }
    ScalarField3D<T>* outsideDensityP() { return outsideDensity_; }

    Dot3D const& absOffset() const { return absoluteOffset; }
    Dot3D const& rhoBarOffset() const { return relativeOffsetRhoBar; }
    Dot3D const& jOffset() const { return relativeOffsetJ; }
    Dot3D const& massOffset() const { return relativeOffsetMass; }
    Dot3D const& volumeFractionOffset() const { return relativeOffsetVF; }
    Dot3D const& flagOffset() const { return relativeOffsetFS; }
    Dot3D const& normalOffset() const { return relativeOffsetNormal; }
    Dot3D const& curvatureOffset() const { return relativeOffsetC; }
    Dot3D const& outsideDensityOffset() const { return relativeOffsetOD; }
...
template<typename T>
std::auto_ptr<MultiScalarField3D<T> > computeNormalizedVolumeFraction(MultiScalarField3D<T>& volumeFraction, Box3D domain)
{
    std::auto_ptr<MultiScalarField3D<T> > result = extractSubDomain<T>(volumeFraction, domain);
    boundScalarField(*result, domain, (T) 0, (T) 1);
    return result;
}

template<typename T>
std::auto_ptr<MultiScalarField3D<T> > computeNormalizedVolumeFraction(MultiScalarField3D<T>& volumeFraction)
{
