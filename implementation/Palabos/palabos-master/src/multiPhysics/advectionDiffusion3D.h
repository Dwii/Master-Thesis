...
public:
    ...
    virtual MaskedTurbulentLatticeToPassiveAdvDiff3D<T,FluidDescriptor,ScalarDescriptor>* clone() const;
    virtual BlockDomain::DomainT appliesTo() const;
    void getTypeOfModification(std::vector<modif::ModifT>& modified) const;
private:
    T C;
...
public:
    ...
    virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> blocks);
    virtual N_MaskedTurbulentLatticeToPassiveAdvDiff3D<T,FluidDescriptor,ScalarDescriptor>* clone() const;
    virtual BlockDomain::DomainT appliesTo() const;
    void getTypeOfModification(std::vector<modif::ModifT>& modified) const;
private:
    T C;
...
public:
    // Pr_t is the turbulent Prandtl number (the ratio between the eddy kinematic viscosity and the eddy diffusivity).
    AdvectionDiffusionFreeSurfaceCoupling3D(Dynamics<T,AD_Descriptor>* conductiveDynamics_,
...
public:
    // Pr_t is the turbulent Prandtl number (the ratio between the eddy kinematic viscosity and the eddy diffusivity).
    MaskedAdvectionDiffusionFreeSurfaceCoupling3D(Dynamics<T,AD_Descriptor>* conductiveDynamics_,
    ...
    virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> blocks);
    virtual MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T,AD_Descriptor,FS_Descriptor>* clone() const;
    virtual void getTypeOfModification(std::vector<modif::ModifT>& modified) const;
private:
    Dynamics<T,AD_Descriptor>* conductiveDynamics;
    Dynamics<T,AD_Descriptor>* adiabaticDynamics;
...
public:
    // Pr_t is the turbulent Prandtl number (the ratio between the eddy kinematic viscosity and the eddy diffusivity).
    N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D(Dynamics<T,AD_Descriptor>* conductiveDynamics_,
            Dynamics<T,AD_Descriptor>* adiabaticDynamics_, T iniVal_, T Pr_t, int maskValue_);
    ...
    N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T,AD_Descriptor,FS_Descriptor>& operator=(
            N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T,AD_Descriptor,FS_Descriptor> const& rhs);
    void swap(N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T,AD_Descriptor,FS_Descriptor>& rhs);
    virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> blocks);
    virtual N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T,AD_Descriptor,FS_Descriptor>* clone() const;
    virtual void getTypeOfModification(std::vector<modif::ModifT>& modified) const;
private:
    Dynamics<T,AD_Descriptor>* conductiveDynamics;
    Dynamics<T,AD_Descriptor>* adiabaticDynamics;
...
public:
    MaskedVariableDiffusivityAdvectionDiffusionFd3D(ADCellT adCellT_, T defaultValue_,
            Box3D const& fullClosedDomain_, bool xPeriodic_, bool yPeriodic_, bool zPeriodic_,
            bool upwind_ = true);
    virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> fields);
...
public:
    ...
    virtual void process(Box3D domain, BlockLattice3D<T,FluidDescriptor>& fluid, ScalarField3D<T>& diffusivity);
...
// It couples a number of species (nSpecies) according to a number of chemical reactions (nReactions).
// It is very important to note that all data provided to this data processor are in physical units
// (as opposed to lattice units). This is because the units of the reaction rate contstant depend on
// the corresponding global order of reaction, so it is more convenient to do all calculations in physical
// units, compute the source terms of the involved advection-diffusion equations and then scale back
// to lattice units only these source terms. The units of the different quantities are as follows:
//
...
//
// The time step (dt) is provided in seconds, in order to transform the source terms from physical to
// lattice units.
//
...
public:
    ArrheniusChemicalReactionCoupling3D(plint nSpecies_, plint nReactions_, T dt_,
            std::vector<T> const& preExponentialFactors_, std::vector<T> const& activationEnergies_,
            std::vector<T> const& reactionEnthalpies_, std::vector<std::vector<int> > const& stoichiometryMatrix_,
