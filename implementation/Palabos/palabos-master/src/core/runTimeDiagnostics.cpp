...
{
    if (!empty()) {
        fprintf(stderr, "Errors at process %d: %s", global::mpi().getRank(), allMessages().c_str()); 
...
{
    if (!memoryEmpty()) {
        fprintf(stderr, "Memory errors at process %d: %s", global::mpi().getRank(), allMemoryMessages().c_str()); 
        fflush(stderr); // This is not really needed since stderr is either line buffered or unbuffered.
    }
}
...
{
    if (!ioEmpty()) {
        fprintf(stderr, "IO errors at process %d: %s", global::mpi().getRank(), allIOMessages().c_str()); 
        fflush(stderr); // This is not really needed since stderr is either line buffered or unbuffered.
    }
}
...
{
    if (!networkEmpty()) {
        fprintf(stderr, "Network errors at process %d: %s", global::mpi().getRank(), allNetworkMessages().c_str()); 
        fflush(stderr); // This is not really needed since stderr is either line buffered or unbuffered.
    }
}
...
{
    if (!logicEmpty()) {
        fprintf(stderr, "Logic errors at process %d: %s", global::mpi().getRank(), allLogicMessages().c_str()); 
        fflush(stderr); // This is not really needed since stderr is either line buffered or unbuffered.
    }
}
...
{
    if (!outOfRangeEmpty()) {
        fprintf(stderr, "Out-of-range errors at process %d: %s", global::mpi().getRank(), allOutOfRangeMessages().c_str()); 
        fflush(stderr); // This is not really needed since stderr is either line buffered or unbuffered.
    }
}
