...
public:
    ...
    virtual void execute(plint iX, plint iY, Cell<T,Descriptor>& cell) const;
    virtual SetCustomBoundaryTemperatureFunctional2D<T,Descriptor,TemperatureFunction>* clone() const;
private:
...
public:
    ...
    virtual void execute(plint iX, plint iY, Cell<T,Descriptor>& cell) const;
    virtual IniCustomThermalEquilibriumFunctional2D<T,Descriptor,RhoVelThetaFunction>* clone() const;
    virtual void setscale(int dxScale, int dtScale);
private:
...
template<typename T, template<typename U> class Descriptor>
class SetExternalScalarFromScalarFieldFunctional2D : public BoxProcessingFunctional2D_LS<T,Descriptor,T> {
public:
    SetExternalScalarFromScalarFieldFunctional2D(int whichScalar_);
    virtual void process(Box2D domain, BlockLattice2D<T,Descriptor>& lattice, ScalarField2D<T> &field);
    virtual BlockDomain::DomainT appliesTo() const;
    virtual void getTypeOfModification(std::vector<modif::ModifT>& modified) const;
...
public:
    SetCustomExternalVectorFunctional2D (
...
class SetExternalVectorFromTensorFieldFunctional2D : public BoxProcessingFunctional2D_LT<T,Descriptor, T, nDim> {
public:
    SetExternalVectorFromTensorFieldFunctional2D (
            int vectorStartsAt_ );
    virtual void process(Box2D domain, BlockLattice2D<T,Descriptor>& lattice, TensorField2D<T,nDim>& tensor);
    virtual BlockDomain::DomainT appliesTo() const;
    virtual void getTypeOfModification(std::vector<modif::ModifT>& modified) const;
