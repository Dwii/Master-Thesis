...
template<typename T, template<typename U> class Descriptor>
void StoreDynamicsFunctional3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
...
template<typename T, template<typename U> class Descriptor>
ExtractDynamicsChainFunctional3D<T,Descriptor>* ExtractDynamicsChainFunctional3D<T,Descriptor>::clone() const {
    return new ExtractDynamicsChainFunctional3D<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor>
void ExtractDynamicsChainFunctional3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
}
...
template<typename T, template<typename U> class Descriptor>
void ExtractTopMostDynamicsFunctional3D<T,Descriptor>::
                      ...
                      ScalarField3D<int>& mask )
{
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                mask.get(iX,iY,iZ) = getBottomMostDynamics(lattice.get(iX,iY,iZ).getDynamics()).getId();
            }
        }
    }
}
...
template<typename T, template<typename U> class Descriptor>
void ExtractBottomMostDynamicsFunctional3D<T,Descriptor>::
    getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
        ...
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                destinationLattice.attributeDynamics (
                        iX_,iY_,iZ_, meta::dynamicsRegistration<T,Descriptor>().
                                         generate(unserializer) );
...
template<typename T, template<typename U> class Descriptor>
AssignEntireCellFunctional3D<T,Descriptor>*
    AssignEntireCellFunctional3D<T,Descriptor>::clone() const
{
    return new AssignEntireCellFunctional3D<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor>
void AssignEntireCellFunctional3D<T,Descriptor>::
    getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
