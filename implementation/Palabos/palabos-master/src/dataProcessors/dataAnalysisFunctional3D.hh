        ...
        Box3D domain, BlockLattice3D<T,Descriptor>& lattice, TensorField3D<T,Descriptor<T>::d>& force )
{
    ...
    static const Array<plint,3> dummy((plint) 0, (plint) 0, (plint) 0);
    static const int mEBounceBackId = MomentumExchangeBounceBack<T,Descriptor>(dummy).getId();
    static const int specularReflectionId = SpecularReflection<T,Descriptor>().getId();
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                int dynamicsId = lattice.get(iX,iY,iZ).getDynamics().getId();
                bool considerForce = (dynamicsId != bounceBackId && dynamicsId != noDynamicsId &&
                                      dynamicsId != mEBounceBackId && dynamicsId != specularReflectionId);
                Array<T,Descriptor<T>::d> velocity;
                lattice.get(iX,iY,iZ).computeVelocity(velocity);
                if (considerForce) {
                    Array<T,Descriptor<T>::d> const& f = force.get(iX+offset.x,iY+offset.y,iZ+offset.z);
...
template<typename T, template<typename U> class Descriptor> 
void BoxSumForcedEnergyFunctional3D<T,Descriptor>::getDimensionsX(std::vector<int>& dimensions) const
{
    dimensions.resize(1);
    dimensions[0] = 2;
}
...
template<typename T, template<typename U> class Descriptor> 
void BoxSumForcedEnergyFunctional3D<T,Descriptor>::getDimensionsT(std::vector<int>& dimensions) const
{
    dimensions.resize(1);
    dimensions[0] = -2;
}
...
template<typename T, template<typename U> class Descriptor> 
BoxSumConstForcedEnergyFunctional3D<T,Descriptor>::BoxSumConstForcedEnergyFunctional3D(Array<T,Descriptor<T>::d> force_)
    : force(force_),
      sumEnergyId(this->getStatistics().subscribeSum())
{ }
...
template<typename T, template<typename U> class Descriptor> 
void BoxSumConstForcedEnergyFunctional3D<T,Descriptor>::getDimensionsX(std::vector<int>& dimensions) const
{
    dimensions.resize(1);
    dimensions[0] = 2;
}
...
template<typename T, template<typename U> class Descriptor> 
void BoxSumConstForcedEnergyFunctional3D<T,Descriptor>::getDimensionsT(std::vector<int>& dimensions) const
{
    dimensions.resize(1);
    dimensions[0] = -2;
}
...
template<typename T>
ScalarFieldSingleProbe3D<T>::ScalarFieldSingleProbe3D (
...
template<typename T>
void ScalarFieldSingleProbe3D<T>::process (
    ...
    for (pluint i=0; i<positions.size(); ++i) {
        ...
        if (contained(referenceCellPos, domain)) {
            ...
            for (plint iCell=0; iCell<8; ++iCell) {
                T cellScalar = scalarField.get(cellPos[iCell].x,cellPos[iCell].y,cellPos[iCell].z);
...
template<typename T>
ScalarFieldSingleProbe3D<T>*
    ScalarFieldSingleProbe3D<T>::clone() const
{
...
template<typename T, template<typename U> class Descriptor>
InternalDensitySingleProbe3D<T,Descriptor>*
    InternalDensitySingleProbe3D<T,Descriptor>::clone() const
{
...
InternalVelocitySingleProbe3D<T,Descriptor>::InternalVelocitySingleProbe3D (
        std::vector<Array<T,3> > const& positions_, 
        std::vector<plint> const& velIds_ )
...
template<typename T, template<typename U> class Descriptor>
InternalVelocitySingleProbe3D<T,Descriptor>*
    InternalVelocitySingleProbe3D<T,Descriptor>::clone() const
{
...
template<typename T, template<typename U> class Descriptor>
void BoxJfunctional3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;  // lattice
...
template<typename T, template<typename U> class Descriptor> 
BoxRhoBarJPiNeqfunctional3D<T,Descriptor>* BoxRhoBarJPiNeqfunctional3D<T,Descriptor>::clone() const
{
    return new BoxRhoBarJPiNeqfunctional3D<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor> 
void BoxRhoBarJPiNeqfunctional3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;  // lattice
    modified[1] = modif::staticVariables;   // rhoBar
    modified[2] = modif::staticVariables;   // j
...
template<typename T>
void VelocityFromRhoBarAndJfunctional3D<T>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::staticVariables;   // velocity
...
template<typename T, template<typename U> class Descriptor> 
void BoxForcedVelocityNormFunctional3D<T,Descriptor>::processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> blocks)
{
    static const int bounceBackId = BounceBack<T,Descriptor>().getId();
    static const int noDynamicsId = NoDynamics<T,Descriptor>().getId();
    static const Array<plint,3> dummy((plint) 0, (plint) 0, (plint) 0);
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                lattice->get(iX,iY,iZ).computeVelocity(velocity);
                if (considerForce) {
                    Array<T,Descriptor<T>::d> const& f = force->get(iX+ofsF.x,iY+ofsF.y,iZ+ofsF.z);
                    velocity[0] += (T) 0.5 * f[0];
                    velocity[1] += (T) 0.5 * f[1];
                    velocity[2] += (T) 0.5 * f[2];
                }
                ...
                // Otherwise, the call to std::sqrt would fail, because std::sqrt is overloaded, but not
                // for Palabos' Complex type.
                velocityNorm->get(iX+ofsV.x,iY+ofsV.y,iZ+ofsV.z)
                    = std::sqrt( (typename PlbTraits<T>::BaseType) VectorTemplate<T,Descriptor>::normSqr(velocity) );
            }
        }
    }
...
template<typename T, template<typename U> class Descriptor> 
BoxForcedVelocityNormFunctional3D<T,Descriptor>* BoxForcedVelocityNormFunctional3D<T,Descriptor>::clone() const
{
    return new BoxForcedVelocityNormFunctional3D<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor> 
void BoxForcedVelocityNormFunctional3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;
...
template<typename T, template<typename U> class Descriptor> 
BoxConstForcedVelocityNormFunctional3D<T,Descriptor>::BoxConstForcedVelocityNormFunctional3D(Array<T,Descriptor<T>::d> force_)
    : force(force_)
{ }
...
template<typename T, template<typename U> class Descriptor> 
BoxConstForcedVelocityNormFunctional3D<T,Descriptor>* BoxConstForcedVelocityNormFunctional3D<T,Descriptor>::clone() const
{
    return new BoxConstForcedVelocityNormFunctional3D<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor> 
void BoxConstForcedVelocityNormFunctional3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
}
...
template<typename T, template<typename U> class Descriptor> 
BoxForcedVelocityComponentFunctional3D<T,Descriptor>::BoxForcedVelocityComponentFunctional3D(int iComponent_)
    : iComponent(iComponent_)
{ }

template<typename T, template<typename U> class Descriptor> 
void BoxForcedVelocityComponentFunctional3D<T,Descriptor>::processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> blocks)
{
    static const int bounceBackId = BounceBack<T,Descriptor>().getId();
    static const int noDynamicsId = NoDynamics<T,Descriptor>().getId();
    static const Array<plint,3> dummy((plint) 0, (plint) 0, (plint) 0);
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                if (considerForce) {
                    Array<T,Descriptor<T>::d> const& f = force->get(iX+ofsF.x,iY+ofsF.y,iZ+ofsF.z);
                    velocityComponent->get(iX+ofsV.x,iY+ofsV.y,iZ+ofsV.z) = velocity[iComponent] + (T) 0.5 * f[iComponent];
                } else {
                    velocityComponent->get(iX+ofsV.x,iY+ofsV.y,iZ+ofsV.z) = velocity[iComponent];
                }
            }
        }
    }
...
template<typename T, template<typename U> class Descriptor> 
BoxForcedVelocityComponentFunctional3D<T,Descriptor>* BoxForcedVelocityComponentFunctional3D<T,Descriptor>::clone() const
{
    return new BoxForcedVelocityComponentFunctional3D<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor> 
void BoxForcedVelocityComponentFunctional3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;
    modified[1] = modif::nothing;
    modified[2] = modif::staticVariables;
}
...
template<typename T, template<typename U> class Descriptor> 
BoxConstForcedVelocityComponentFunctional3D<T,Descriptor>::BoxConstForcedVelocityComponentFunctional3D(
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                lattice.get(iX,iY,iZ).computeVelocity(velocity);
                if (considerForce) {
                    scalarField.get(iX+offset.x,iY+offset.y,iZ+offset.z) = velocity[iComponent] + (T) 0.5 * force[iComponent];
                } else {
                    scalarField.get(iX+offset.x,iY+offset.y,iZ+offset.z) = velocity[iComponent];
                }
            }
...
template<typename T, template<typename U> class Descriptor> 
BoxConstForcedVelocityComponentFunctional3D<T,Descriptor>* BoxConstForcedVelocityComponentFunctional3D<T,Descriptor>::clone() const
{
    return new BoxConstForcedVelocityComponentFunctional3D<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor> 
void BoxConstForcedVelocityComponentFunctional3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
}
...
template<typename T, template<typename U> class Descriptor> 
void BoxForcedVelocityFunctional3D<T,Descriptor>::processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> blocks)
{
    static const int bounceBackId = BounceBack<T,Descriptor>().getId();
    static const int noDynamicsId = NoDynamics<T,Descriptor>().getId();
    static const Array<plint,3> dummy((plint) 0, (plint) 0, (plint) 0);
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                Array<T,Descriptor<T>::d>& v = velocity->get(iX+ofsV.x,iY+ofsV.y,iZ+ofsV.z);
                if (considerForce) {
                    Array<T,Descriptor<T>::d> const& f = force->get(iX+ofsF.x,iY+ofsF.y,iZ+ofsF.z);
                    v[0] = u[0] + (T) 0.5 * f[0];
                    v[1] = u[1] + (T) 0.5 * f[1];
...
template<typename T, template<typename U> class Descriptor> 
BoxForcedVelocityFunctional3D<T,Descriptor>* BoxForcedVelocityFunctional3D<T,Descriptor>::clone() const
{
    return new BoxForcedVelocityFunctional3D<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor> 
void BoxForcedVelocityFunctional3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
    modified[0] = modif::nothing;
    modified[1] = modif::nothing;
    modified[2] = modif::staticVariables;
...
template<typename T, template<typename U> class Descriptor> 
BoxConstForcedVelocityFunctional3D<T,Descriptor>::BoxConstForcedVelocityFunctional3D(Array<T,Descriptor<T>::d> force_)
    : force(force_)
{ }
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                Array<T,Descriptor<T>::d> u;
                lattice.get(iX,iY,iZ).computeVelocity(u);
                Array<T,Descriptor<T>::d>& v = tensorField.get(iX+offset.x,iY+offset.y,iZ+offset.z);
                if (considerForce) {
...
template<typename T, template<typename U> class Descriptor> 
BoxConstForcedVelocityFunctional3D<T,Descriptor>* BoxConstForcedVelocityFunctional3D<T,Descriptor>::clone() const
{
    return new BoxConstForcedVelocityFunctional3D<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor> 
void BoxConstForcedVelocityFunctional3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
}
...
BoxStressFunctional3D<T,Descriptor>::BoxStressFunctional3D(T rho0_, bool isCompressible_)
    : rho0(rho0_),
      isCompressible(isCompressible_)
{ }
...
template<typename T, template<typename U> class Descriptor> 
void BoxStressFunctional3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
}
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                shearRate.get(iX + offset.x, iY + offset.y, iZ + offset.z) = normPiNeq / (tau * Descriptor<T>::cs2 * rho);
            }
        }
    }
}
...
template<typename T, template<typename U> class Descriptor> 
void BoxShearRateFunctional3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
}
...
template<typename T, template<typename U> class Descriptor>
BoxNTensorShearRateFunctional3D<T,Descriptor>* BoxNTensorShearRateFunctional3D<T,Descriptor>::clone() const
{
    return new BoxNTensorShearRateFunctional3D<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor>
void BoxNTensorShearRateFunctional3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
}
        ...
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                if (!util::isZero(omega)) {
                    scalarField.get(iX+offset.x,iY+offset.y,iZ+offset.z) = Descriptor<T>::cs2*((T)1/omega-(T)0.5);
                } else {
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                T omega = lattice.get(iX,iY,iZ).getDynamics().getOmega();
                if (!util::isZero(omega)) {
                    *nu.get(iX+offset.x,iY+offset.y,iZ+offset.z) = Descriptor<T>::cs2*((T)1/omega-(T)0.5);
                } else {
...
template<typename T, template<typename U> class Descriptor>
BoxNTensorKinematicViscosityFunctional3D<T,Descriptor>* BoxNTensorKinematicViscosityFunctional3D<T,Descriptor>::clone() const
{
    return new BoxNTensorKinematicViscosityFunctional3D<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor>
void BoxNTensorKinematicViscosityFunctional3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
}
...
template<typename T, template<typename U> class Descriptor>
TagLocalDynamicsFunctional3D<T,Descriptor>::TagLocalDynamicsFunctional3D(int dynamicsId_, int tag_)
    : dynamicsId(dynamicsId_),
      tag(tag_)
{ }

template<typename T, template<typename U> class Descriptor>
void TagLocalDynamicsFunctional3D<T,Descriptor>::process(Box3D domain, BlockLattice3D<T,Descriptor>& lattice,
        ScalarField3D<int>& tags)
{
...
template<typename T, template<typename U> class Descriptor>
TagLocalDynamicsFunctional3D<T,Descriptor>* TagLocalDynamicsFunctional3D<T,Descriptor>::clone() const
{
...
template<typename T, template<typename U> class Descriptor>
void TagLocalDynamicsFunctional3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
    modified[0] = modif::nothing;           // Lattice.
...
template<typename T>
void BoxScalarIntSumFunctional3D<T>::process (
        Box3D domain, ScalarField3D<T>& scalarField )
{
    BlockStatistics& statistics = this->getStatistics();
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
...
template<typename T>
void BoundScalarField3D<T>::process (
        Box3D domain, ScalarField3D<T>& data )
{
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
...
template<typename T, template<typename U> class Descriptor>
void LBMsmoothenInPlace3D<T,Descriptor>::process (
        Box3D domain, ScalarField3D<T>& data )
{
    typedef Descriptor<T> D;
...
template<typename T, template<typename U> class Descriptor>
LBMsmoothenInPlace3D<T,Descriptor>*
    LBMsmoothenInPlace3D<T,Descriptor>::clone() const
{
                      ...
                      TensorField3D<T,nDim>& tensorField )
{
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                scalarField.get(iX,iY,iZ) = maxElement(tensorField.get(iX+offset.x,iY+offset.y,iZ+offset.z));
            }
        }
    }
}
...
template<typename T, int nDim>
BoxLocalMaximumPerComponentFunctional3D<T,nDim>* BoxLocalMaximumPerComponentFunctional3D<T,nDim>::clone() const
{
    return new BoxLocalMaximumPerComponentFunctional3D<T,nDim>(*this);
}

template<typename T, int nDim>
void BoxLocalMaximumPerComponentFunctional3D<T,nDim>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::staticVariables;
    modified[1] = modif::nothing;
}
...
template<typename T, int nDim>
BoxBulkVorticityOrderFourFunctional3D<T,nDim>* BoxBulkVorticityOrderFourFunctional3D<T,nDim>::clone() const
{
    return new BoxBulkVorticityOrderFourFunctional3D<T,nDim>(*this);
}

template<typename T, int nDim>
void BoxBulkVorticityOrderFourFunctional3D<T,nDim>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
}
...
template<typename T, int nDim>
BoxBulkVorticityOrderSixFunctional3D<T,nDim>* BoxBulkVorticityOrderSixFunctional3D<T,nDim>::clone() const
{
    return new BoxBulkVorticityOrderSixFunctional3D<T,nDim>(*this);
}

template<typename T, int nDim>
void BoxBulkVorticityOrderSixFunctional3D<T,nDim>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
}
...
template<typename T, int nDim>
BoxBulkVorticityOrderEightFunctional3D<T,nDim>* BoxBulkVorticityOrderEightFunctional3D<T,nDim>::clone() const
{
    return new BoxBulkVorticityOrderEightFunctional3D<T,nDim>(*this);
}

template<typename T, int nDim>
void BoxBulkVorticityOrderEightFunctional3D<T,nDim>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
}
...
void BoxBulkHelicityFunctional3D<T,nDim>::process (
        Box3D domain, ScalarField3D<T>& helicity,
        ...
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            ...
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                helicity.get(iX,iY,iZ) =
                    velocity.get(iX2,iY2,iZ2)[0] * fdDataField::bulkVorticityX(velocity, iX2,iY2,iZ2) +
                    velocity.get(iX2,iY2,iZ2)[1] * fdDataField::bulkVorticityY(velocity, iX2,iY2,iZ2) +
                    velocity.get(iX2,iY2,iZ2)[2] * fdDataField::bulkVorticityZ(velocity, iX2,iY2,iZ2);
            }
        }
    }
}
...
template<typename T, int nDim>
void BoxBulkHelicityFunctional3D<T,nDim>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::staticVariables;
    modified[1] = modif::nothing;
}
...
void BoxHelicityFunctional3D<T,nDim>::processPlane (
        int direction, int orientation, Box3D domain,
        ScalarField3D<T>& helicity, TensorField3D<T,nDim>& velocity )
{
    Dot3D offset = computeRelativeDisplacement(helicity, velocity);
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                plint iZ2 = iZ+offset.z;
                helicity.get(iX,iY,iZ) = 
                    velocity.get(iX2,iY2,iZ2)[0] * fdDataField::planeVorticityX(velocity,direction,orientation, iX2,iY2,iZ2) +
                    velocity.get(iX2,iY2,iZ2)[1] * fdDataField::planeVorticityY(velocity,direction,orientation, iX2,iY2,iZ2) +
                    velocity.get(iX2,iY2,iZ2)[2] * fdDataField::planeVorticityZ(velocity,direction,orientation, iX2,iY2,iZ2);
            }
        }
    }
}
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                plint iZ2 = iZ+offset.z;
                helicity.get(iX,iY,iZ) = 
                    velocity.get(iX2,iY2,iZ2)[0] * fdDataField::edgeVorticityX(velocity,plane,normal1,normal2, iX2,iY2,iZ2) +
                    velocity.get(iX2,iY2,iZ2)[1] * fdDataField::edgeVorticityY(velocity,plane,normal1,normal2, iX2,iY2,iZ2) +
                    velocity.get(iX2,iY2,iZ2)[2] * fdDataField::edgeVorticityZ(velocity,plane,normal1,normal2, iX2,iY2,iZ2);
            }
        }
    }
}
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                plint iZ2 = iZ+offset.z;
                helicity.get(iX,iY,iZ) = 
                    velocity.get(iX2,iY2,iZ2)[0] * fdDataField::cornerVorticityX(velocity,normalX,normalY,normalZ, iX2,iY2,iZ2) +
                    velocity.get(iX2,iY2,iZ2)[1] * fdDataField::cornerVorticityY(velocity,normalX,normalY,normalZ, iX2,iY2,iZ2) +
                    velocity.get(iX2,iY2,iZ2)[2] * fdDataField::cornerVorticityZ(velocity,normalX,normalY,normalZ, iX2,iY2,iZ2);
            }
        }
    }
}
...
template<typename T, int nDim>
void BoxHelicityFunctional3D<T,nDim>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::staticVariables;
    modified[1] = modif::nothing;
}
...
template<typename T, int nDim>
void InterpolateTensorFieldFunctional3D<T,nDim>::processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> blocks)
{
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                res.resetToZero();
                for (plint iField = 0; iField < N; iField++) {
                    res += w[iField] * fields[iField]->get(iX + ofsF[iField].x, iY + ofsF[iField].y, iZ + ofsF[iField].z);
                }
            }
        }
    }
...
template<typename T, int nDim>
InterpolateTensorFieldFunctional3D<T,nDim>* InterpolateTensorFieldFunctional3D<T,nDim>::clone() const
{
    return new InterpolateTensorFieldFunctional3D<T,nDim>(*this);
}

template<typename T, int nDim>
void InterpolateTensorFieldFunctional3D<T,nDim>::getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
    std::fill(modified.begin(), modified.end(), modif::nothing);    // All tensor fields that participate in the interpolation.
...
template<typename T, int nDim>
Tensor_A_dividedBy_Scalar_B_inplace_functional3D<T,nDim>* Tensor_A_dividedBy_Scalar_B_inplace_functional3D<T,nDim>::clone() const {
    return new Tensor_A_dividedBy_Scalar_B_inplace_functional3D<T,nDim>(*this);
}

template<typename T, int nDim>
void Tensor_A_dividedBy_Scalar_B_inplace_functional3D<T,nDim>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::staticVariables;
    modified[1] = modif::nothing;
}
...
template<typename T, int nDim>
Masked_Tensor_A_dividedBy_Scalar_B_inplace_functional3D<T,nDim>::Masked_Tensor_A_dividedBy_Scalar_B_inplace_functional3D(int flag_)
    : flag(flag_)
{ }
...
template<typename T, int nDim>
BlockDomain::DomainT Masked_Tensor_A_dividedBy_Scalar_B_inplace_functional3D<T,nDim>::appliesTo() const
{
    return BlockDomain::bulk;
}
    ...
    Box3D domain, TensorField3D<T,nDim> &tensor, TensorField3D<T,nDim> &avg)
{
    ...
    for (plint iX = domain.x0; iX <= domain.x1; iX++) {
        for (plint iY = domain.y0; iY <= domain.y1; iY++) {
            for (plint iZ = domain.z0; iZ <= domain.z1; iZ++) {
                for (plint iD = 0; iD < nDim; ++iD) {
                    avg.get(iX + offset.x, iY + offset.y, iZ + offset.z)[iD] = oneOverN *
                        (nMinusOne * avg.get(iX + offset.x, iY + offset.y, iZ + offset.z)[iD] + tensor.get(iX, iY, iZ)[iD]);
                }
            }
        }
    }
...
template<typename T, int nDim>
UpdateAveTensorTransientStatistics3D<T,nDim>* UpdateAveTensorTransientStatistics3D<T,nDim>::clone() const
{
    return new UpdateAveTensorTransientStatistics3D<T,nDim>(*this);
}

template<typename T, int nDim>
void UpdateAveTensorTransientStatistics3D<T,nDim>::getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
...
template<typename T, int nDim>
void UpdateDevTensorTransientStatistics3D<T, nDim>::processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> blocks)
{
    PLB_ASSERT(blocks.size() == 3);
    ...
    TensorField3D<T,(nDim*(nDim+1))/2> *statistics = dynamic_cast<TensorField3D<T,(nDim*(nDim+1))/2>*>(blocks[2]);
    PLB_ASSERT(tensor);
    PLB_ASSERT(average);
    PLB_ASSERT(statistics);
    ...
    for (plint iX = domain.x0; iX <= domain.x1; iX++) {
        for (plint iY = domain.y0; iY <= domain.y1; iY++) {
            for (plint iZ = domain.z0; iZ <= domain.z1; iZ++) {
                plint iPi = 0;
                for (plint iA = 0; iA < nDim; ++iA) {
                    T newDataA = tensor->get(iX, iY, iZ)[iA] - average->get(iX + ofsA.x, iY + ofsA.y, iZ + ofsA.z)[iA];

                    for (plint iB = iA; iB < nDim; ++iB) {
                        T newDataB = tensor->get(iX, iY, iZ)[iB] - average->get(iX + ofsA.x, iY + ofsA.y, iZ + ofsA.z)[iB];

                        T oldDev = statistics->get(iX + ofsS.x, iY + ofsS.y, iZ + ofsS.z)[iPi];
                        statistics->get(iX + ofsS.x, iY + ofsS.y, iZ + ofsS.z)[iPi] = oneOverN *
                            (nMinusOne * oldDev + newDataA * newDataB);
...
template<typename T, int nDim>
UpdateDevTensorTransientStatistics3D<T, nDim>* UpdateDevTensorTransientStatistics3D<T, nDim>::clone() const
{
    return new UpdateDevTensorTransientStatistics3D<T, nDim>(*this);
}

template<typename T, int nDim>
void UpdateDevTensorTransientStatistics3D<T, nDim>::getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
    modified[0] = modif::nothing;           // Tensor field.
    modified[1] = modif::nothing;           // Mean value field.
    modified[2] = modif::staticVariables;   // Statistics field.
...
void ExtractNTensorSubDomainFunctional3D<T>::process (
        Box3D domain, NTensorField3D<T>& field1,
                      NTensorField3D<T>& field2 )
{
        ...
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                for (plint iDim=0; iDim<field1.getNdim(); ++iDim) {
                    field2.get(iX+offset.x,iY+offset.y,iZ+offset.z)[iDim] = field1.get(iX,iY,iZ)[iDim];
                }
            }
        }
    }
...
template<typename T>
void ExtractNTensorSubDomainFunctional3D<T>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing;
    modified[1] = modif::staticVariables;
}
...
template<typename T>
MaskedNTensorNeumannInLayersFunctional3D<T>::MaskedNTensorNeumannInLayersFunctional3D(int toFlag_, int fromFlag_, int noFlag_)
    : toFlag(toFlag_),
      fromFlag(fromFlag_),
      noFlag(noFlag_)
{
...
template<typename T>
void MaskedNTensorNeumannInLayersFunctional3D<T>::process(Box3D domain, NTensorField3D<T>& data, NTensorField3D<int>& mask)
{
                    ...
                    for (plint dx = -1; dx <= +1; dx++) {
                        ...
                        for (plint dy = -1; dy <= +1; dy++) {
                            ...
                            for (plint dz = -1; dz <= +1; dz++) {
                                plint nextZ = iZ + dz;
                                if (*mask.get(nextX + offset.x, nextY + offset.y, nextZ + offset.z) == fromFlag) {
...
template<typename T>
MaskedNTensorNeumannInLayersFunctional3D<T>* MaskedNTensorNeumannInLayersFunctional3D<T>::clone() const
{
    return new MaskedNTensorNeumannInLayersFunctional3D<T>(*this);
}

template<typename T>
void MaskedNTensorNeumannInLayersFunctional3D<T>::getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
    modified[0] = modif::staticVariables;
    modified[1] = modif::staticVariables;
}
