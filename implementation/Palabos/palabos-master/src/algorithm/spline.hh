...
T NaturalCubicSpline<T>::getIntegralValue(T tmin, T tmax) const
{
    ...
    T leftIntegral = (T) 0;
    T rightIntegral = (T) 0;
    if (util::lessThan(tmin, x[0]) && util::lessThan(tmax, x[0])) {
        return (tmax - tmin) * y[0];
    } else if (util::lessThan(tmin, x[0]) && util::greaterEqual(tmax, x[0])) {
        leftIntegral = (x[0] - tmin) * y[0];
    } else if (util::greaterThan(tmax, x[n-1]) && util::greaterThan(tmin, x[n-1])) {
        return (tmax - tmin) * y[n-1];
    } else if (util::greaterThan(tmax, x[n-1]) && util::lessEqual(tmin, x[n-1])) {
        rightIntegral = (tmax - x[n-1]) * y[n-1];
    }
