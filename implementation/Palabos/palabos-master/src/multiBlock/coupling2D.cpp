...
Coupling2D::Coupling2D( DataProcessorGenerator2D* generator_, std::vector<plint> const& multiBlocks_,
...
CouplingAction2D::CouplingAction2D(Coupling2D* coupling_)
    : coupling(coupling_)
{ }
...
EvaluateStatsAction2D::EvaluateStatsAction2D(plint blockId_)
    : blockId(blockId_)
{ }
                               ...
                               std::vector<plint> blockNums, Box2D domain )
{
    ...
    Coupling2D* coupling = new Coupling2D (
        new BoxProcessorGenerator2D(functional, domain), blockNums, allMultiBlocks );
    actions.push_back(new CouplingAction2D(coupling));
    return (plint)actions.size()-1;
}

plint Actions2D::addProcessor( BoxProcessingFunctional2D* functional,
                               plint blockNum1, Box2D domain )
{
...
plint Actions2D::addProcessor( BoxProcessingFunctional2D* functional,
                               plint blockNum1, plint blockNum2, Box2D domain )
{
    std::vector<plint> blockNums;
    blockNums.push_back(blockNum1);
