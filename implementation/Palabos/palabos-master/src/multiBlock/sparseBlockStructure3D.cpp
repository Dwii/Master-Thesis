...
SparseBlockStructure3D::SparseBlockStructure3D(Box3D boundingBox_)
...
void SparseBlockStructure3D::addBlock(Box3D const& bulk,
                                      Box3D const& uniqueBulk, plint blockId)
{
...
std::vector<plint> SparseBlockStructure3D::getLocalBlocks(ThreadAttribution const& attribution) const
{
...
{
    return Box3D ( gridPosX(realBlock.x0), gridPosX(realBlock.x1),
...
SparseBlockStructure3D scale(SparseBlockStructure3D const& sparseBlock, plint relativeLevel)
{
    ...
    for (; bulks!=sparseBlock.getBulks().end(); ++bulks) {
        ...
        newSparseBlock.addBlock (
            ...
            global::getDefaultMultiScaleManager().scaleBox(uniqueBulk, relativeLevel),
            id );
    }
...
SparseBlockStructure3D block_union( SparseBlockStructure3D const& sparseBlock1,
                                    ...
                                    std::map<plint,std::vector<plint> >& remappedIds )
{
    Box3D newBoundingBox = bound(sparseBlock1.getBoundingBox(),
                                 sparseBlock2.getBoundingBox());
    SparseBlockStructure3D newSparseBlock(newBoundingBox);
