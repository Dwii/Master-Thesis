...
SparseBlockStructure2D::SparseBlockStructure2D(Box2D boundingBox_)
...
void SparseBlockStructure2D::addBlock(Box2D const& bulk,
                                      Box2D const& uniqueBulk, plint blockId)
{
...
std::vector<plint> SparseBlockStructure2D::getLocalBlocks(ThreadAttribution const& attribution) const
{
...
{
    return Box2D ( gridPosX(realBlock.x0), gridPosX(realBlock.x1),
                   gridPosY(realBlock.y0), gridPosY(realBlock.y1) );
}
        ...
        for (plint dy=-1; dy<=+1; ++dy) {
            if (!(dx==0 && dy==0)) {
                computeOverlaps (
                    ...
                    ids, overlapsOnBulk, overlapsOnNeighbors );
            }
        }
    }
}
...
SparseBlockStructure2D intersect( SparseBlockStructure2D const& sparseBlock,
...
SparseBlockStructure2D intersect( SparseBlockStructure2D const& sparseBlock,
                                  Box2D domain, Box2D newBoundingBox )
{
 ...
 **/
SparseBlockStructure2D intersect( SparseBlockStructure2D const& sparseBlock1,
...
SparseBlockStructure2D extend( SparseBlockStructure2D const& sparseBlock,
                               Box2D addedBulk, Box2D addedUniqueBulk,
 ...
 **/
SparseBlockStructure2D except( SparseBlockStructure2D const& sparseBlock,
                               Box2D exceptedBlock,
 ...
 **/
SparseBlockStructure2D block_union( SparseBlockStructure2D const& sparseBlock1,
                                    SparseBlockStructure2D const& sparseBlock2,
                                    std::map<plint,std::vector<plint> >& remappedIds )
{
    Box2D newBoundingBox = bound(sparseBlock1.getBoundingBox(),
                                 sparseBlock2.getBoundingBox());
    SparseBlockStructure2D newSparseBlock(newBoundingBox);
    ...
    for (; partnerIt != partnerBulks.end(); ++partnerIt) {
        ...
        for (pluint iInters=0; iInters<intersections.size(); ++iInters) {
            ...
            for (pluint iExc=0; iExc<exceptedBlocks.size(); ++iExc) {
                ...
                PLB_ASSERT( doesIntersect );
                newSparseBlock.addBlock( exceptedBlock, exceptedUniqueBulk,
                                         newSparseBlock.nextIncrementalId() );
            }
        }
