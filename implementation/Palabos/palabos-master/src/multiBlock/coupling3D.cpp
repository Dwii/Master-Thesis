...
Coupling3D::Coupling3D( DataProcessorGenerator3D* generator_, std::vector<plint> const& multiBlocks_,
...
CouplingAction3D::CouplingAction3D(Coupling3D* coupling_)
    : coupling(coupling_)
{ }
...
ReductiveCouplingAction3D& ReductiveCouplingAction3D::operator=(ReductiveCouplingAction3D const& rhs)
{
...
ExecuteAllInternalProcAction3D::ExecuteAllInternalProcAction3D(plint blockId_)
    : blockId(blockId_)
{ }
...
SubActionsAction3D::SubActionsAction3D(Actions3D* actions_)
    : actions(actions_)
{ }
...
EvaluateStatsAction3D::EvaluateStatsAction3D(plint blockId_)
    : blockId(blockId_)
{ }
...
plint Actions3D::addProcessor( BoxProcessingFunctional3D* functional,
                               std::vector<plint> blockNums, Box3D domain )
{
    for (pluint i=0; i<blockNums.size(); ++i) {
        PLB_ASSERT( blockNums[i] < (plint)allMultiBlocks.size() );
    }
    Coupling3D* coupling = new Coupling3D (
        new BoxProcessorGenerator3D(functional, domain), blockNums, allMultiBlocks );
    actions.push_back(new CouplingAction3D(coupling));
    return (plint)actions.size()-1;
}

plint Actions3D::addProcessor( BoxProcessingFunctional3D* functional,
                               std::vector<plint> blockNums, std::vector<Box3D> const& domains )
{
    for (pluint i=0; i<blockNums.size(); ++i) {
        PLB_ASSERT( blockNums[i] < (plint)allMultiBlocks.size() );
    }
...
plint Actions3D::addProcessor( BoxProcessingFunctional3D* functional,
                               plint blockNum1, std::vector<Box3D> const& domains )
{
    std::vector<plint> blockNums;
    blockNums.push_back(blockNum1);
...
plint Actions3D::addProcessor( BoxProcessingFunctional3D* functional,
                               plint blockNum1, plint blockNum2, Box3D domain )
{
    std::vector<plint> blockNums;
    blockNums.push_back(blockNum1);
...
plint Actions3D::addProcessor( BoxProcessingFunctional3D* functional, plint blockNum1,
                               plint blockNum2, std::vector<Box3D> const& domains )
{
    std::vector<plint> blockNums;
    blockNums.push_back(blockNum1);
    blockNums.push_back(blockNum2);
...
plint Actions3D::addProcessor( BoxProcessingFunctional3D* functional,
                               plint blockNum1, plint blockNum2, plint blockNum3,
                               Box3D domain )
{
    std::vector<plint> blockNums;
    blockNums.push_back(blockNum1);
...
plint Actions3D::addProcessor( ReductiveBoxProcessingFunctional3D* functional,
                               std::vector<plint> blockNums, Box3D domain )
{
    for (pluint i=0; i<blockNums.size(); ++i) {
        PLB_ASSERT( blockNums[i] < (plint)allMultiBlocks.size() );
...
plint Actions3D::addProcessor( ReductiveBoxProcessingFunctional3D* functional,
                               plint blockNum1, plint blockNum2, plint blockNum3, Box3D domain )
{
    std::vector<plint> blockNums;
    blockNums.push_back(blockNum1);
    blockNums.push_back(blockNum2);
...
BlockStatistics const& Actions3D::statistics(plint actionId) const {
    PLB_ASSERT( actionId < (plint)actions.size() );
    ReductiveCouplingAction3D* action = dynamic_cast<ReductiveCouplingAction3D*>(actions[actionId]);
    PLB_ASSERT( action );
