...
const int MultiBlockLattice2D<T,Descriptor>::staticId =
...
template<typename T, template<typename U> class Descriptor>
MultiBlockLattice2D<T,Descriptor>::MultiBlockLattice2D(MultiBlock2D const& rhs, Box2D subDomain, bool crop)
...
MultiBlockLattice2D<T,Descriptor>*
    MultiBlockLattice2D<T,Descriptor>::clone() const
{
...
template<typename T, template<typename U> class Descriptor>
MultiBlockLattice2D<T,Descriptor>*
    MultiBlockLattice2D<T,Descriptor>::clone(MultiBlockManagement2D const& newManagement) const
{
...
void MultiBlockLattice2D<T,Descriptor>::streamImplementation() {
          ...
          it != blockLattices.end(); ++it)
    {
        ...
        Box2D domain = extendPeriodic(bulk.computeNonPeriodicEnvelope(),
...
template<typename T, template<typename U> class Descriptor>
void MultiBlockLattice2D<T,Descriptor>::getDynamicsDict(Box2D domain, std::map<std::string,int>& dict)
{
    std::vector<int> ids;
...
template<typename T, template<typename U> class Descriptor>
BlockLattice2D<T,Descriptor> const& MultiBlockLattice2D<T,Descriptor>::getComponent(plint blockId) const {
