...
template<typename T>
void TriangleToDef<T>::vsAdd (
        Array<T,3> const& coord, plint& index, plint& count )
{
...
void TriangleToDef<T>::bvmLabel()
{
    ...
    for (; it !=boundaryVertexMap.end(); ++it) {
        ...
        localEdge2 = -1; // Local index (in the triangle t2) of the
                         ...
                         // `vertex2'
        if (vertex == edgeList[3*triangle2 + 2].pv &&
            vertex2 == edgeList[3*triangle2].pv)
        {
            localEdge2 = 0;
        }
        else if (vertex == edgeList[3*triangle2].pv &&
                 vertex2 == edgeList[3*triangle2 + 1].pv)
        {
            localEdge2 = 1;
        }
        else if (vertex == edgeList[3*triangle2 + 1].pv &&
                 vertex2 == edgeList[3*triangle2 + 2].pv)
        {
            localEdge2 = 2;
        }
        else {
    ...
    for (plint iTriangle = 0; iTriangle < numTriangles; iTriangle++) {
        if (visitedTriangles[iTriangle] == 0) {
            ...
            while (!trianglesToFixNeighbors.empty()) {
                ...
                fixOrientationOfNeighbors(triangle, trianglesToFixNeighbors, visitedTriangles,
                                          fixedSomething);
            }
        }
    }
...
void TriangleToDef<T>::fixOrientationOfNeighbors(plint iTriangle,
    ...
    plint ind[3] = { globalVertex(iTriangle, 0),
                     globalVertex(iTriangle, 1),
                     globalVertex(iTriangle, 2) };
        ...
        for(plint iEdge=0; iEdge<(plint)edgeTable[iVertex].size(); ++iEdge) {
            ...
            if (triangle2 != -1) { /* Internal edge */
                ...
                computeInternalNeighboringEdges (
                        iVertex, jVertex, triangle1, triangle2,
                        localEdge1, va1, vb1 );
            }
            else { /* Boundary edge */
...
void TriangleToDef<T>::computeEmanatingEdges()
{
    ...
    for (plint iTriangle = 0; iTriangle < numTriangles; ++iTriangle) {
        for (plint localEdge = 0; localEdge < 3; ++localEdge) {
            plint& emanatingEdge =
                emanatingEdgeList[globalVertex(iTriangle, localEdge)];
            if (emanatingEdge == -1) {
