    ...
    if (this->isSolid(cellLocation+offset)) {
        for (int iNeighbor=0; iNeighbor<NextNeighbor<T>::numNeighbors; ++iNeighbor) {
            ...
            if (this->isFluid(neighbor+offset)) {
                ...
                if (!ok) {
                    global::plbErrors().registerError("Guo off-lattice model could not find an intersection with a triangle.");
                }
        ...
        std::vector<AtomicBlock3D *> const& args )
{
    ...
    if (dryNodes.size() != dryNodeFluidDirections.size()) {
        global::plbErrors().registerError("Error in the Guo off-lattice model boundary completion.");
    }
...
bool GuoAlgorithm3D<T,Descriptor>::computeNeighborData()
{
    ...
    for (plint iDirection=0; iDirection<numDirections; ++iDirection) {
        ...
        if (!(wallDistance <= NextNeighbor<T>::d[iNeighbor])) {
            global::plbErrors().registerError("Error treating the geometry in the Guo off-lattice model.");
        }
...
public:
    ...
    GuoOffPopAlgorithm3D (
        ...
        std::vector<std::pair<int,int> > const& dryNodeFluidDirections_,
        std::vector<plint> const& dryNodeIds_, Dot3D const& absoluteOffset_,
        Array<T,3>& localForce_, std::vector<AtomicBlock3D *> const& args_, bool computeStat_, bool secondOrder_ );
    virtual void extrapolateVariables (
              Dot3D const& fluidDirection, int depth, Array<T,3> const& wallNode, T delta,
              Array<T,3> const& wall_vel, OffBoundary::Type bdType,
              Array<T,3> const& wallNormal, plint triangleId, plint iDirection );
...
void GuoDefineVelocityAlgorithm3D<T,Descriptor>::complete() {
    if (this->args.empty()) {
        Cell<T,Descriptor> & cell =
            this->lattice.get( this->guoNode.x,
                               this->guoNode.y,
                               this->guoNode.z );

            cell.getDynamics().computeEquilibria( cell.getRawPopulations(),  this->rhoBar, this->j, normSqr(this->j));
    } else {
        if ((plint) this->args.size() == 1) {
            ...
            T * macroscopic = macroField->get (
                    this->guoNode.x+offset.x,
                    this->guoNode.y+offset.y,
                    this->guoNode.z+offset.z);
            ...
            rhoBarField->get(
                    this->guoNode.x+offsetScalar.x,
                    this->guoNode.y+offsetScalar.y,
                    this->guoNode.z+offsetScalar.z) = this->rhoBar;
            Dot3D offset = computeRelativeDisplacement(this->lattice, *jField);
            jField->get( this->guoNode.x+offset.x, 
                         this->guoNode.y+offset.y, 
...
public:
    ...
    GuoFdCompletionAlgorithm3D (
        ...
        Array<T,3>& localForce_, std::vector<AtomicBlock3D *> const& args_,
        bool computeStat_, bool secondOrder_, 
        std::pair<int,int> const &xDerivDirAndOrder, 
        std::pair<int,int> const &yDerivDirAndOrder, 
        std::pair<int,int> const &zDerivDirAndOrder );
    virtual void extrapolateVariables (
              Dot3D const& fluidDirection, int depth, Array<T,3> const& wallNode, T delta,
              Array<T,3> const& wall_vel, OffBoundary::Type bdType,
              Array<T,3> const& wallNormal, plint triangleId, plint iDirection );
...
private:
    ...
    void computeCentralGradient(NTensorField3D<T> const* macroField, const Dot3D &pos, 
        const Dot3D &dx, Array<T,Descriptor<T>::d> &d_u);
...
GuoFdCompletionAlgorithm3D<T,Descriptor>::GuoFdCompletionAlgorithm3D (
            ...
            Array<T,3>& localForce_, std::vector<AtomicBlock3D *> const& args_,
            bool computeStat_, bool secondOrder_,
            std::pair<int,int> const &xDerivDirAndOrder_, 
            std::pair<int,int> const &yDerivDirAndOrder_, 
            std::pair<int,int> const &zDerivDirAndOrder_  )
    : GuoAlgorithm3D<T,Descriptor> (
            model_, lattice_, guoNode_, dryNodeFluidDirections_, dryNodeIds_,
            absoluteOffset_, localForce_, args_, computeStat_, secondOrder_ ),
    xDerivDirAndOrder(xDerivDirAndOrder_), yDerivDirAndOrder(yDerivDirAndOrder_), 
...
{ 
    if (this->args.empty()) {
        ...
        if ((plint) this->args.size() == 1) {
            ...
            PLB_ASSERT( macroField->getNdim()==4 );
            Dot3D offset = computeRelativeDisplacement(this->lattice, *macroField);
            T const* macro_0 = macroField->get( this->guoNode.x+offset.x, this->guoNode.y+offset.y, this->guoNode.z+offset.z);
            ...
            this->j = jField->get( this->guoNode.x+offset.x, this->guoNode.y+offset.y, this->guoNode.z+offset.z);
            this->rhoBar = rhoBarField->get( this->guoNode.x+offsetScalar.x, this->guoNode.y+offsetScalar.y, this->guoNode.z+offsetScalar.z);
        } else {
            PLB_ASSERT(false); // Not implemented for 3 args.
        }
    }
...
template<typename T, template<typename U> class Descriptor>
void GuoFdCompletionAlgorithm3D<T,Descriptor>::
...
void GuoFdCompletionAlgorithm3D<T,Descriptor>::complete() 
{
    ...
    if (this->args.empty()) {
        if (xDerivDirAndOrder.second == 2) {
            ...
            computeFwdGradient(this->lattice, u0, this->guoNode, Dot3D(xDerivDirAndOrder.first,0,0),xDerivDirAndOrder.first, dx_u);
        ...
        if (yDerivDirAndOrder.second == 2) {
            ...
            computeFwdGradient(this->lattice, u0, this->guoNode, Dot3D(0,yDerivDirAndOrder.first,0),yDerivDirAndOrder.first, dy_u);
        ...
        if (zDerivDirAndOrder.second == 2) {
            ...
            computeFwdGradient(this->lattice, u0, this->guoNode, Dot3D(0,0,zDerivDirAndOrder.first),zDerivDirAndOrder.first, dz_u);
        ...
        if ((plint) this->args.size() == 1) {
            ...
            if (xDerivDirAndOrder.second == 2) {
                ...
                PLB_ASSERT((xDerivDirAndOrder.first == 1 || xDerivDirAndOrder.first == -1) && "Invalid xDerivDirAndOrder.first value (must be = 1 or = -1");
                computeFwdGradient(macroField, u0, this->guoNode+offset, Dot3D(xDerivDirAndOrder.first,0,0),xDerivDirAndOrder.first, dx_u);
            } else if (xDerivDirAndOrder.second == 0) {
                dx_u.resetToZero();
            } else {
                PLB_ASSERT(false && "xDerivDirAndOrder has invalid order");
            ...
            if (yDerivDirAndOrder.second == 2) {
                ...
                PLB_ASSERT((yDerivDirAndOrder.first == 1 || yDerivDirAndOrder.first == -1) && "Invalid yDerivDirAndOrder.first value (must be = 1 or = -1");
                computeFwdGradient(macroField, u0, this->guoNode+offset, Dot3D(0,yDerivDirAndOrder.first,0),yDerivDirAndOrder.first, dy_u);
            } else if (yDerivDirAndOrder.second == 0) {
                dy_u.resetToZero();
            } else {
                PLB_ASSERT(false && "yDerivDirAndOrder has invalid order");
            ...
            if (zDerivDirAndOrder.second == 2) {
                ...
                PLB_ASSERT((zDerivDirAndOrder.first == 1 || zDerivDirAndOrder.first == -1) && "Invalid zDerivDirAndOrder.first value (must be = 1 or = -1");
                computeFwdGradient(macroField, u0, this->guoNode+offset, Dot3D(0,0,zDerivDirAndOrder.first),zDerivDirAndOrder.first, dz_u);
            } else if (zDerivDirAndOrder.second == 0) {
                dz_u.resetToZero();
            } else {
                PLB_ASSERT(false && "zDerivDirAndOrder has invalid order");
            ...
            if (xDerivDirAndOrder.second == 2) {
                computeCentralGradient(rhoBarField, jField, this->guoNode+offset, this->guoNode+offsetScalar, Dot3D(1,0,0), dx_u);
            } else if (xDerivDirAndOrder.second == 1) {
                PLB_ASSERT((xDerivDirAndOrder.first == 1 || xDerivDirAndOrder.first == -1) && "Invalid xDerivDirAndOrder.first value (must be = 1 or = -1");
                computeFwdGradient(rhoBarField, jField, u0, this->guoNode+offset, this->guoNode+offsetScalar, Dot3D(xDerivDirAndOrder.first,0,0),xDerivDirAndOrder.first, dx_u);
            } else if (xDerivDirAndOrder.second == 0) {
                dx_u.resetToZero();
            } else {
                PLB_ASSERT(false && "xDerivDirAndOrder has invalid order");
            ...
            if (yDerivDirAndOrder.second == 2) {
                computeCentralGradient(rhoBarField, jField, this->guoNode+offset, this->guoNode+offsetScalar, Dot3D(0,1,0), dy_u);
            } else if (yDerivDirAndOrder.second == 1) {
                PLB_ASSERT((yDerivDirAndOrder.first == 1 || yDerivDirAndOrder.first == -1) && "Invalid yDerivDirAndOrder.first value (must be = 1 or = -1");
                computeFwdGradient(rhoBarField, jField, u0, this->guoNode+offset, this->guoNode+offsetScalar, Dot3D(0,yDerivDirAndOrder.first,0),yDerivDirAndOrder.first, dy_u);
            } else if (yDerivDirAndOrder.second == 0) {
                dy_u.resetToZero();
            } else {
                PLB_ASSERT(false && "yDerivDirAndOrder has invalid order");
            ...
            if (zDerivDirAndOrder.second == 2) {
                computeCentralGradient(rhoBarField, jField, this->guoNode+offset, this->guoNode+offsetScalar, Dot3D(0,0,1), dz_u);
            } else if (zDerivDirAndOrder.second == 1) {
                PLB_ASSERT((zDerivDirAndOrder.first == 1 || zDerivDirAndOrder.first == -1) && "Invalid zDerivDirAndOrder.first value (must be = 1 or = -1");
                computeFwdGradient(rhoBarField, jField, u0, this->guoNode+offset, this->guoNode+offsetScalar, Dot3D(0,0,zDerivDirAndOrder.first),zDerivDirAndOrder.first, dz_u);
            } else if (zDerivDirAndOrder.second == 0) {
                dz_u.resetToZero();
            } else {
                PLB_ASSERT(false && "zDerivDirAndOrder has invalid order");
...
template<typename T, template<typename U> class Descriptor>
bool GuoOffLatticeFdModel3D<T,Descriptor>::LiquidNeighbor::
         operator<(LiquidNeighbor const& rhs) const
{
    return cosAngle < rhs.cosAngle;
}
...
template<typename T, template<typename U> class Descriptor>
std::pair<int,int> GuoOffLatticeFdModel3D<T,Descriptor>::
...
template<typename T, template<typename U> class Descriptor>
ContainerBlockData*
    GuoOffLatticeFdModel3D<T,Descriptor>::generateOffLatticeInfo() const
{
    return new GuoOffLatticeInfo3D;
}
        ...
        std::vector<AtomicBlock3D *> const& args )
{
    ...
    if (dryNodes.size() != dryNodeFluidDirections.size()) {
        global::plbErrors().registerError("Error in the Guo off-lattice Fd model boundary completion.");
    }
    ...
    for (pluint iDry=0; iDry<dryNodes.size(); ++iDry) {
        cellCompletion (
            ...
            xDerivDirAndOrder[iDry], yDerivDirAndOrder[iDry], zDerivDirAndOrder[iDry], localForce, args );
    }
}
