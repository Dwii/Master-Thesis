        ...
        plint indexOffset, int numDecimalDigits) const
{
    if (global::mpi().isMainProcessor()) {
        ...
        for (plint iVertex = 0; iVertex < numVertices; iVertex++) {
            fprintf(fp, "% .*Le % .*Le % .*Le\n",
                    numDecimalDigits, (long double) (*workingVertices)[iVertex+indexOffset][0],
                    numDecimalDigits, (long double) (*workingVertices)[iVertex+indexOffset][1],
                    numDecimalDigits, (long double) (*workingVertices)[iVertex+indexOffset][2]);
        }
