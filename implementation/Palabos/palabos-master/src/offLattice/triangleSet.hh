...
template<typename T>
TriangleSet<T>::TriangleSet(std::vector<Triangle> const& triangles_, T eps_)
    : minEdgeLength(std::numeric_limits<T>::max()),
      maxEdgeLength(std::numeric_limits<T>::min())
{
    PLB_ASSERT(eps_ > (T) 0);
...
template<typename T>
TriangleSet<T>::TriangleSet(std::string fname, T eps_, SurfaceGeometryFileFormat fformat)
    : minEdgeLength(std::numeric_limits<T>::max()),
      maxEdgeLength(std::numeric_limits<T>::min())
{
    PLB_ASSERT(eps_ > (T) 0);
...
{
    plbIOErrorIfCannotOpenFileForReading(fname);
...
template<typename T>
void TriangleSet<T>::writeAsciiSTL(std::string fname, int numDecimalDigits) const
{
    if (global::mpi().isMainProcessor()) {
        ...
        for (plint i = 0; i < size; i++) {
            ...
            Array<T,3> n = computeTriangleNormal(v0, v1, v2, isAreaWeighted);

            fprintf(fp, "  facet normal % .*Le % .*Le % .*Le\n",
                    d, (long double) n[0], d, (long double) n[1], d, (long double) n[2]);
            fprintf(fp, "    outer loop\n");
            fprintf(fp, "      vertex % .*Le % .*Le % .*Le\n",
                    d, (long double) v0[0], d, (long double) v0[1], d, (long double) v0[2]);
            fprintf(fp, "      vertex % .*Le % .*Le % .*Le\n",
                    d, (long double) v1[0], d, (long double) v1[1], d, (long double) v1[2]);
            fprintf(fp, "      vertex % .*Le % .*Le % .*Le\n",
                    d, (long double) v2[0], d, (long double) v2[1], d, (long double) v2[2]);
            fprintf(fp, "    endloop\n");
            fprintf(fp, "  endfacet\n");
        }
