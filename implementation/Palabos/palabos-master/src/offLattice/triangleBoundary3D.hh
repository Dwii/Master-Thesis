...
template<typename T, class SurfaceData>
BoundaryProfiles3D<T,SurfaceData>::BoundaryProfiles3D(BoundaryProfiles3D<T,SurfaceData> const& rhs)
{
    wallProfile = rhs.wallProfile->clone();
    typename std::map<plint,BoundaryProfile3D<T,SurfaceData>*>::const_iterator it = rhs.profiles.begin();
    for (; it!=rhs.profiles.end(); ++it) {
        profiles.insert (
                std::pair<plint,BoundaryProfile3D<T,SurfaceData>*> (
...
template<typename T, class SurfaceData>
void BoundaryProfiles3D<T,SurfaceData>::setWallProfile(BoundaryProfile3D<T,SurfaceData>* wallProfile_)
{
...
template<typename T, class SurfaceData>
void BoundaryProfiles3D<T,SurfaceData>::resetProfiles(std::map<plint,BoundaryProfile3D<T,SurfaceData>*> profiles_)
{
        ...
        std::vector<BoundaryProfile3D<T,SurfaceData>*> inletOutlets )
{
    ...
    for (pluint iProfile=0; iProfile<inletOutlets.size(); ++iProfile) {
        plint id = inletOutletIds[iProfile];
#ifdef PLB_DEBUG
        typename std::map<plint,BoundaryProfile3D<T,SurfaceData>*>::const_iterator it = newProfiles.find(id);
...
template<typename T>
TriangleBoundary3D<T> const&
    TriangleBoundary3D<T>::select (
...
template<typename T>
TriangleBoundary3D<T> const&
    TriangleBoundary3D<T>::pushSelect (
            plint whichTopology, plint whichVertices ) const
{
    PLB_PRECONDITION( whichTopology==0 || whichTopology==1 );
    PLB_PRECONDITION( whichVertices>=0 && whichVertices < (plint)vertexLists.size() );
...
template<typename T>
TriangleBoundary3D<T> const&
    TriangleBoundary3D<T>::popSelect() const
{
...
template<typename T>
void TriangleBoundary3D<T>::getSelection (
        plint& whichTopology, plint& whichVertices ) const
{
...
template<typename T>
VertexProperty3D<T> const*
    TriangleBoundary3D<T>::getVertexProperty(plint iVertex) const
{
...
std::vector<Lid> const&
    TriangleBoundary3D<T>::getInletOutlet() const
{
...
template<typename T>
std::vector<Lid>
    TriangleBoundary3D<T>::getInletOutlet(plint sortDirection) const
{
    PLB_PRECONDITION( topology.top()==1 );
...
template<typename T>
template<typename DomainFunctional>
plint TriangleBoundary3D<T>::tagDomain(DomainFunctional functional, Array<T,3> normal, T angleTolerance, plint previousTag)
{
    // Make sure we're working with the closed mesh.
    PLB_PRECONDITION( topology.top()==1 );
    ++currentTagNum;
...
template< typename T, class SurfaceData >
TriangleFlowShape3D<T,SurfaceData>*
    TriangleFlowShape3D<T,SurfaceData>::clone() const
{
...
template< typename T, class SurfaceData >
TriangleFlowShape3D<T,SurfaceData>*
    TriangleFlowShape3D<T,SurfaceData>::clone (
            std::vector<AtomicBlock3D*> args ) const
{
    PLB_PRECONDITION( args.size()==3 );
    TriangleFlowShape3D<T,SurfaceData>*
        newShape = new TriangleFlowShape3D<T,SurfaceData>(*this);
...
VoxelizedDomain3D<T>::VoxelizedDomain3D (
        TriangleBoundary3D<T> const& boundary_,
        int flowType_, Box3D const& boundingBox, plint borderWidth_,
        plint envelopeWidth_, plint blockSize_, Box3D const& seed, plint gridLevel_, bool dynamicMesh_ )
    : flowType(flowType_),
      borderWidth(borderWidth_),
      boundary(boundary_)
{
...
MultiScalarField3D<int>&
    VoxelizedDomain3D<T>::getVoxelMatrix()
{
    return *voxelMatrix;
}

template<typename T>
MultiScalarField3D<int> const&
    VoxelizedDomain3D<T>::getVoxelMatrix() const
{
    return *voxelMatrix;
}
...
MultiContainerBlock3D&
    VoxelizedDomain3D<T>::getTriangleHash()
{
    return *triangleHash;
}
...
void VoxelizedDomain3D<T>::reparallelize(MultiBlockRedistribute3D const& redistribute) {
    MultiBlockManagement3D newManagement = redistribute.redistribute(voxelMatrix->getMultiBlockManagement());
    MultiScalarField3D<int>* newVoxelMatrix =
...
template<typename T>
AddLayerFunctional3D<T>::AddLayerFunctional3D(T previousLayer_)
    : previousLayer(previousLayer_)
{ }

template<typename T>
void AddLayerFunctional3D<T>::process (
