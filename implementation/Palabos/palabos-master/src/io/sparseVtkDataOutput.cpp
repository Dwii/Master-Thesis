...
{
// If we are in a Unix-like or a Windows system we create a directory, called dName, to store the atomic block vti files.
// rdName is the relative path of dName, from where the vtm files are stored.
#ifdef PLB_USE_POSIX
...
#else
    vtmFile << "<VTKFile type=\"vtkMultiBlockDataSet\" version=\"1.0\" byte_order=\"LittleEndian\">\n";
#endif
        ...
        plint vtkBlockId, bool pointData )
{
    vtmFile << "<Block index=\"" << vtkBlockId << "\" "
            << "name=\""
            << createFileName("Level ", vtkBlockId, 3)
            << "\">\n";
