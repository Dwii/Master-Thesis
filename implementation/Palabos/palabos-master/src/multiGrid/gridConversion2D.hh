...
ScalarCopyFineToCoarseZerothOrder2D<T>::ScalarCopyFineToCoarseZerothOrder2D(
        ...
        : dimDx(rhs.dimDx), dimDt(rhs.dimDt),
          dxScale(rhs.dxScale), dtScale(rhs.dtScale)
{ }
        
template<typename T>
ScalarCopyFineToCoarseZerothOrder2D<T>& 
        ...
        Box2D fineDomain, ScalarField2D<T>& fineField, ScalarField2D<T>& coarseField )
{
    ...
    Box2D coarseDomain (
            fineDomain.shift(posFine.x,posFine.y).        // Convert to absolute fine coordinates.
                divideAndFitSmaller(stretch).                        // Rescale, but don't exceed original domain.
    ...
    for (plint coarseX=coarseDomain.x0; coarseX<=coarseDomain.x1; ++coarseX, fineX+=stretch) {
        ...
        for (plint coarseY=coarseDomain.y0; coarseY<=coarseDomain.y1; ++coarseY, fineY+=stretch) {
            coarseField.get(coarseX,coarseY) =
                    fineField.get(fineX,fineY)*scaleFactor;
        }
    }
}
...
template<typename T, int nDim>
TensorCopyFineToCoarseZerothOrder2D<T,nDim>* TensorCopyFineToCoarseZerothOrder2D<T,nDim>::clone() const{
...
void LatticeCopyFineToCoarseZerothOrder2D<T,Descriptor>::process (
        Box2D fineDomain,
        BlockLattice2D<T,Descriptor>& fineLattice, BlockLattice2D<T,Descriptor>& coarseLattice )
{
...
template<typename T, template<typename U> class Descriptor>
LatticeCopyFineToCoarseZerothOrder2D<T,Descriptor>* LatticeCopyFineToCoarseZerothOrder2D<T,Descriptor>::clone() const{
...
template<typename T>
ScalarInterpolateCoarseToFine2D<T>& 
...
void ScalarInterpolateCoarseToFine2D<T>::process (
        Box2D coarseDomain,
        ScalarField2D<T>& coarseField, ScalarField2D<T>& fineField )
{
    Dot2D posFine = fineField.getLocation();
    Dot2D posCoarse = coarseField.getLocation();
            ...
            plint fineY = (coarseY+posCoarse.y)*stretch - posFine.y;
                ...
                for (plint iY=fineY-stretch/2+1; iY<=fineY+stretch/2; ++iY) {
                    if (contained(iX,iY,fineField.getBoundingBox())) {
                        ...
                        linearInterpolationCoefficients(coarseField, Array<T,2> (
...
template<typename T, int nDim>
TensorInterpolateCoarseToFine2D<T,nDim>& 
...
void TensorInterpolateCoarseToFine2D<T,nDim>::process (
        Box2D coarseDomain,
        TensorField2D<T,nDim>& coarseField, TensorField2D<T,nDim>& fineField )
{
    Dot2D posFine = fineField.getLocation();
    Dot2D posCoarse = coarseField.getLocation();
        ...
        plint dimDx, plint dimDt, plint dxScale, plint dtScale )
{
    ...
    MultiBlockManagement2D management = scaleMultiBlockManagement(fineField.getMultiBlockManagement(),relativeLevel);
