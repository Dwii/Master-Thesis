        ...
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                if (originalDynamics->isNonLocal()) {
                    NonLocalBoundaryDynamics3D<T,Descriptor>* dynamics = (NonLocalBoundaryDynamics3D<T,Descriptor>*)(originalDynamics);
        ...
        MultiBlockLattice3D<T,Descriptor>& lattice, Box3D bbox, Box3D domain, boundary::BcType bcType )
{
    ...
    switch (bcType) {
        ...
        case boundary::freeslip: {
                ...
                dynamicsTemplate = new NLD_VelocityNeumannBoundaryDynamics3D<T,Descriptor>(baseDynamics, noPenetration);
            }
            break;
...
void setNLDdynamics( MultiBlockLattice3D<T,Descriptor>& lattice, Box3D domain,
                     NonLocalBoundaryDynamics3D<T,Descriptor>* dynamics ) {
