...
public:
    ...
    virtual void planeComputeMacroscopic (
            ...
            T& rhoBar, Array<T,3>& j, T& thetaBar ) =0;
...
public:
            ...
            T& rhoBar, Array<T,3>& j, T& thetaBar )
    {
        NonLocalBoundaryDynamics3D<T,Descriptor>* dynamics =
            dynamic_cast<NonLocalBoundaryDynamics3D<T,Descriptor>*>(&lattice.get(iX,iY,iZ).getDynamics());
        if (dynamics) {
            dynamics->planeComputeMacroscopic(direction, orientation, iX, iY, iZ, lattice, rhoBar, j, thetaBar);
        }
        else {
            rhoBar = T();
            j.resetToZero();
            ...
            T& rhoBar, Array<T,3>& j, T& thetaBar )
    {
        NonLocalBoundaryDynamics3D<T,Descriptor>* dynamics =
            dynamic_cast<NonLocalBoundaryDynamics3D<T,Descriptor>*>(&lattice.get(iX,iY,iZ).getDynamics());
        if (dynamics) {
            dynamics->edgeComputeMacroscopic(plane, normal1, normal2, iX, iY, iZ, lattice, rhoBar, j, thetaBar);
        }
        else {
            rhoBar = T();
            j.resetToZero();
            ...
            T& rhoBar, Array<T,3>& j, T& thetaBar )
    {
        NonLocalBoundaryDynamics3D<T,Descriptor>* dynamics =
            dynamic_cast<NonLocalBoundaryDynamics3D<T,Descriptor>*>(&lattice.get(iX,iY,iZ).getDynamics());
        if (dynamics) {
            dynamics->cornerComputeMacroscopic(xNormal, yNormal, zNormal, iX, iY, iZ, lattice, rhoBar, j, thetaBar);
        }
        else {
            rhoBar = T();
            j.resetToZero();
...
public:
    ...
    Array<T,3> const& get_u() const { return u; }
public:
    virtual void planeBoundaryCompletion (
            int direction, int orientation, plint iX, plint iY, plint iZ,
            BlockLattice3D<T,Descriptor>& lattice );
...
{
public:
    NLD_VelocityNeumannBoundaryDynamics3D(Dynamics<T,Descriptor>* baseDynamics_, bool noPenetration_ = false);
