...
public:
    ...
    void computeMacroscopic(Array<T,3> const& u, T& rhoBar, Array<T,3>& j) const
    {
        ...
        rhoBar = flatWall::computeRhoBar<T,Descriptor> (
                cell, direction, orientation, uNormal, fNormal );
        j = u;
        if (!dynamics.velIsJ()) {
    ...
    Array<T,3> extrapolateVelocity() const {
        Array<T,3> uTot; uTot.resetToZero();
        static T weight0 = 1./9.;
        static T weight1 = 1./36.;
        static T weight2 = 4./9.;
        static T weights[] = { weight1, weight0, weight1,
                               weight0, weight2, weight0,
                               weight1, weight0, weight1 };
        plint i=0;
        plint numCells=0;
        T sumWeights = T();
        for (plint i1=-1; i1<=+1; ++i1) {
            for (plint i2=-1; i2<=+1; ++i2) {
                ...
                if (!neighborCell.getDynamics().isBoundary())  {
                    Array<T,3> u; neighborCell.computeVelocity(u);
                    uTot += weights[i]*u;
                    ++numCells;
...
public:
    ...
    T getNeighborRhoBar(plint step1, plint step2)
    {
        ...
        if (step1==0) {
            NonLocalBoundaryDynamics3D<T,Descriptor>::staticPlaneComputeMacroscopic (
                    direction1, normal1, coords[0], coords[1], coords[2], lattice, rhoBar, dummyJ, dummyTheta );
            PLB_ASSERT(step2 != 0);
        }
        else {  // step2==0.
            NonLocalBoundaryDynamics3D<T,Descriptor>::staticPlaneComputeMacroscopic (
                    direction2, normal2, coords[0], coords[1], coords[2], lattice, rhoBar, dummyJ, dummyTheta );
        }
        return rhoBar;
    }
    ...
    void computeGradientIntoPlane(Array<T,3> const& u0, int deriveDirection, int deriveOrientation,
                                  int normalDirection, int normalOrientation, Array<T,3>& gradient)
    {
        Array<T,3> u1;
        ...
        T rhoBar, dummyTheta;
        NonLocalBoundaryDynamics3D<T,Descriptor>::staticPlaneComputeMacroscopic (
                normalDirection, normalOrientation, pos1[0], pos1[1], pos1[2], lattice, rhoBar, u1, dummyTheta );
        if (!dynamics.velIsJ()) {
...
public:
    ...
    void computeVelocityGradientX(Array<T,3> const& u0, Array<T,3>& gradient) {
        ...
        NonLocalBoundaryDynamics3D<T,Descriptor>::staticEdgeComputeMacroscopic (
                0, yNormal, zNormal, iX -1*xNormal, iY -0*yNormal, iZ -0*zNormal, lattice, rhoBar, u1, dummyTheta );
        if (!dynamics.velIsJ()) {
            u1 /= Descriptor<T>::fullRho(rhoBar);
        }
        for (int iD=0; iD<Descriptor<T>::d; ++iD) {
    ...
    void computeVelocityGradientY(Array<T,3> const& u0, Array<T,3>& gradient) {
        ...
        T rhoBar, dummyTheta;
        NonLocalBoundaryDynamics3D<T,Descriptor>::staticEdgeComputeMacroscopic (
                1, zNormal, xNormal, iX -0*xNormal, iY -1*yNormal, iZ -0*zNormal, lattice, rhoBar, u1, dummyTheta );
        if (!dynamics.velIsJ()) {
            u1 /= Descriptor<T>::fullRho(rhoBar);
        }
        for (int iD=0; iD<Descriptor<T>::d; ++iD) {
    ...
    void computeVelocityGradientZ(Array<T,3> const& u0, Array<T,3>& gradient) {
        ...
        T rhoBar, dummyTheta;
        NonLocalBoundaryDynamics3D<T,Descriptor>::staticEdgeComputeMacroscopic (
                2, xNormal, yNormal, iX -0*xNormal, iY -0*yNormal, iZ -1*zNormal, lattice, rhoBar, u1, dummyTheta );
        if (!dynamics.velIsJ()) {
            u1 /= Descriptor<T>::fullRho(rhoBar);
        }
        for (int iD=0; iD<Descriptor<T>::d; ++iD) {
    ...
    void computeMacroscopic(Array<T,3> const& u, T& rhoBar, Array<T,3>& j)
    {
        ...
        Array<T,3> dummyJ;
        NonLocalBoundaryDynamics3D<T,Descriptor>::staticEdgeComputeMacroscopic (
                0, yNormal, zNormal, iX -1*xNormal, iY -0*yNormal, iZ -0*zNormal, lattice, rho100, dummyJ, dummyTheta );
        NonLocalBoundaryDynamics3D<T,Descriptor>::staticEdgeComputeMacroscopic (
                1, zNormal, xNormal, iX -0*xNormal, iY -1*yNormal, iZ -0*zNormal, lattice, rho010, dummyJ, dummyTheta );
        NonLocalBoundaryDynamics3D<T,Descriptor>::staticEdgeComputeMacroscopic (
                2, xNormal, yNormal, iX -0*xNormal, iY -0*yNormal, iZ -1*zNormal, lattice, rho001, dummyJ, dummyTheta );
...
template<typename T, template<typename U> class Descriptor>
NLD_VelocityBoundaryDynamics3D<T,Descriptor>::NLD_VelocityBoundaryDynamics3D(Dynamics<T,Descriptor>* baseDynamics_)
    : NonLocalBoundaryDynamics3D<T,Descriptor>(baseDynamics_)
{ }

template<typename T, template<typename U> class Descriptor>
NLD_VelocityBoundaryDynamics3D<T,Descriptor>::NLD_VelocityBoundaryDynamics3D(HierarchicUnserializer& unserializer)
...
{
    NLD_VelocityPlaneUtil3D<T,Descriptor> (
            direction, orientation, iX,iY,iZ, lattice, *this ).computeMacroscopic(this->u, rhoBar, j);
    thetaBar = T();
}
...
void NLD_VelocityBoundaryDynamics3D<T,Descriptor>::cornerComputeMacroscopic (
            ...
            T& rhoBar, Array<T,3>& j, T& thetaBar )
{
    NLD_VelocityCornerUtil3D<T,Descriptor> cornerUtil(xNormal, yNormal, zNormal, iX,iY,iZ, lattice, *this);
            ...
            BlockLattice3D<T,Descriptor>& lattice )
{
    ...
    //      dynamics, and not to NLD dynamics. That's how, in the code below, periodicity is detected.
    ...
    bool isWall1 =
        lattice.get(nbCell1[0],nbCell1[1],nbCell1[2]).getDynamics().isBoundary();
    bool isWall2 =
        lattice.get(nbCell2[0],nbCell2[1],nbCell2[2]).getDynamics().isBoundary();

    // If both directions are periodic, there is no need for a boundary condition.
    if (!isWall1 && !isWall2) return;
            ...
            BlockLattice3D<T,Descriptor>& lattice )
{
    ...
    Array<plint,3> middleNbCell(iX,iY,iZ), nbCellX(iX,iY,iZ), nbCellY(iX,iY,iZ), nbCellZ(iX,iY,iZ);
...
template<typename T, template<typename U> class Descriptor>
int NLD_VelocityNeumannBoundaryDynamics3D<T,Descriptor>::id =
    meta::registerGeneralDynamics<T,Descriptor, NLD_VelocityNeumannBoundaryDynamics3D<T,Descriptor> >
...
template<typename T, template<typename U> class Descriptor>
void NLD_VelocityNeumannBoundaryDynamics3D<T,Descriptor>::prepareCollision(Cell<T,Descriptor>& cell)
{ }
            ...
            T& rhoBar, Array<T,3>& j, T& thetaBar )
{
    ...
    if (!(dyn1 || dyn2)) {
        ...
        // Neighbor1 is Neumann, while neighbor2 is something else --> the outlet normal goes like normal2.
        if (same1 && !same2) {
            T rhoBar_, thetaBar_;
            Array<T,3> j_;
            // Extrapolate the velocity from neighbor2, which is on a plane with normal normal1.
            this->staticPlaneComputeMacroscopic(direction1, normal1, nbCell2[0], nbCell2[1], nbCell2[2], lattice, rhoBar_, j_, thetaBar_);
        ...
        // Neighbor2 is Neumann, while neighbor1 is something else --> the outlet normal goes like normal1.
        if (same2 && !same1) {
            T rhoBar_, thetaBar_;
            Array<T,3> j_;
            // Extrapolate the velocity from neighbor1, which is on a plane with normal normal2.
            this->staticPlaneComputeMacroscopic(direction2, normal2, nbCell1[0], nbCell1[1], nbCell1[2], lattice, rhoBar_, j_, thetaBar_);
            if (!lattice.get(iX,iY,iZ).getDynamics().velIsJ()) {
                j_ *= Descriptor<T>::invRho(rhoBar_);
            }
            edgeUtil.computeMacroscopic(j_, rhoBar, j);
        ...
        Cell<T,Descriptor>& middleNeighbor = lattice.get(middleNbCell[0], middleNbCell[1], middleNbCell[2]);
        Array<T,3> u;
            ...
            T& rhoBar, Array<T,3>& j, T& thetaBar )
{
    ...
    Array<plint,3> nbCell1(iX-xNormal,iY,iZ), nbCell2(iX,iY-yNormal,iZ), nbCell3(iX,iY,iZ-zNormal);
    Cell<T,Descriptor>& neighbor1 = lattice.get(nbCell1[0], nbCell1[1], nbCell1[2]);
    Cell<T,Descriptor>& neighbor2 = lattice.get(nbCell2[0], nbCell2[1], nbCell2[2]);
    ...
    if (!(dyn1 || dyn2 || dyn3)) {
        ...
        // Neighbor 1 and 2 are Neumann, while neighbor 3 is something else --> the outlet normal goes like normal3.
        if (same1 && same2 && !same3) {
            T rhoBar_, thetaBar_;
            Array<T,3> j_;
            // Extrapolate the velocity from neighbor3, which is on an edge in z-direction.
            this->staticEdgeComputeMacroscopic(2, xNormal, yNormal, nbCell3[0], nbCell3[1], nbCell3[2], lattice, rhoBar_, j_, thetaBar_);
            if (!lattice.get(iX,iY,iZ).getDynamics().velIsJ()) {
                j_ *= Descriptor<T>::invRho(rhoBar_);
            }
        ...
        // Neighbor 1 and 3 are Neumann, while neighbor 2 is something else --> the outlet normal goes like normal2.
        if (same1 && same3 && !same2) {
            T rhoBar_, thetaBar_;
            Array<T,3> j_;
            // Extrapolate the velocity from neighbor2, which is on an edge in y-direction.
            this->staticEdgeComputeMacroscopic(1, zNormal, xNormal, nbCell2[0], nbCell2[1], nbCell2[2], lattice, rhoBar_, j_, thetaBar_);
            if (!lattice.get(iX,iY,iZ).getDynamics().velIsJ()) {
                j_ *= Descriptor<T>::invRho(rhoBar_);
            }
            cornerUtil.computeMacroscopic(j_, rhoBar, j);
        ...
        // Neighbor 2 and 3 are Neumann, while neighbor 1 is something else --> the outlet normal goes like normal1.
        if (same2 && same3 && !same1) {
            T rhoBar_, thetaBar_;
            Array<T,3> j_;
            // Extrapolate the velocity from neighbor1, which is on an edge in x-direction.
            this->staticEdgeComputeMacroscopic(0, yNormal, zNormal, nbCell1[0], nbCell1[1], nbCell1[2], lattice, rhoBar_, j_, thetaBar_);
            if (!lattice.get(iX,iY,iZ).getDynamics().velIsJ()) {
                j_ *= Descriptor<T>::invRho(rhoBar_);
            }
            cornerUtil.computeMacroscopic(j_, rhoBar, j);
        ...
        Array<T,3> u; middleNeighbor.computeVelocity(u);
            ...
            BlockLattice3D<T,Descriptor>& lattice )
{
    ...
    if (!(dyn1 || dyn2)) {
        ...
        if (same1 && !same2) {
            ...
            Array<T,3> j_;
            // Extrapolate the velocity from neighbor2, which is on a plane with normal normal1.
            NonLocalBoundaryDynamics3D<T,Descriptor>::staticPlaneComputeMacroscopic(direction1, normal1, nbCell2[0], nbCell2[1], nbCell2[2], lattice, rhoBar_, j_, thetaBar_);
            if (!lattice.get(iX,iY,iZ).getDynamics().velIsJ()) {
                j_ *= Descriptor<T>::invRho(rhoBar_);
            }
            if (noPenetration) {
        ...
        if (same2 && !same1) {
            ...
            Array<T,3> j_;
            // Extrapolate the velocity from neighbor1, which is on a plane with normal normal2.
            NonLocalBoundaryDynamics3D<T,Descriptor>::staticPlaneComputeMacroscopic(direction2, normal2, nbCell1[0], nbCell1[1], nbCell1[2], lattice, rhoBar_, j_, thetaBar_);
            if (!lattice.get(iX,iY,iZ).getDynamics().velIsJ()) {
                j_ *= Descriptor<T>::invRho(rhoBar_);
            }
            if (noPenetration) {
    ...
    // the velocity from the Dirichlet neighbor.
        ...
        if (dyn1) {
            dyn1->edgeBoundaryCompletion (
                        plane, normal1, normal2, iX, iY, iZ, lattice );
        }
        if (dyn2) {
            Cell<T,Descriptor> saveCell(lattice.get(iX,iY,iZ));
            dyn2->edgeBoundaryCompletion (
                        plane, normal1, normal2, iX, iY, iZ, lattice );
            // If both neighbors are Dirichlet, take an average.
            if (dyn1) {
                Cell<T,Descriptor>& cell(lattice.get(iX,iY,iZ));
...
template<typename T, template<typename U> class Descriptor>
NLD_OutflowBoundaryDynamics3D<T,Descriptor>*
    NLD_OutflowBoundaryDynamics3D<T,Descriptor>::clone() const
{
...
void NLD_OutflowBoundaryDynamics3D<T,Descriptor>::edgeComputeMacroscopic (
            ...
            T& rhoBar, Array<T,3>& j, T& thetaBar )
{
    NLD_VelocityNeumannBoundaryDynamics3D<T,Descriptor>(this->getBaseDynamics().clone()).edgeComputeMacroscopic (
...
void NLD_OutflowBoundaryDynamics3D<T,Descriptor>::cornerComputeMacroscopic (
            ...
            T& rhoBar, Array<T,3>& j, T& thetaBar )
{
    NLD_VelocityNeumannBoundaryDynamics3D<T,Descriptor>(this->getBaseDynamics().clone()).cornerComputeMacroscopic (
...
void NLD_OutflowBoundaryDynamics3D<T,Descriptor>::edgeBoundaryCompletion (
            ...
            BlockLattice3D<T,Descriptor>& lattice )
{
    NLD_VelocityNeumannBoundaryDynamics3D<T,Descriptor>(this->getBaseDynamics().clone()).edgeBoundaryCompletion (
            plane, normal1, normal2, iX, iY, iZ, lattice );
}
...
void NLD_OutflowBoundaryDynamics3D<T,Descriptor>::cornerBoundaryCompletion (
            ...
            BlockLattice3D<T,Descriptor>& lattice )
{
    NLD_VelocityNeumannBoundaryDynamics3D<T,Descriptor>(this->getBaseDynamics().clone()).cornerBoundaryCompletion (
            xNormal, yNormal, zNormal, iX, iY, iZ, lattice );
}

template<typename T, template<typename U> class Descriptor>
Dynamics<T,Descriptor>* cloneAndInsertNLDatTop ( Dynamics<T,Descriptor> const& dynamics,
                                                 NonLocalBoundaryDynamics3D<T,Descriptor>* newTop )
{
    ...
    do {
        ...
        if (isComposite) {
            current = &(dynamic_cast<CompositeDynamics<T,Descriptor>*>(current)->getBaseDynamics());
        }
    } while (isComposite);
    PLB_ASSERT( topMost );
    Dynamics<T,Descriptor>* result
        = cloneAndInsertAtTopDynamics(*topMost, newTop);
...
template<typename T, template<typename U> class Descriptor>
NLD_AD_DirichletDynamics3D<T,Descriptor>::NLD_AD_DirichletDynamics3D(Dynamics<T,Descriptor>* baseDynamics_)
    : NonLocalBoundaryDynamics3D<T,Descriptor>(baseDynamics_)
{ }

template<typename T, template<typename U> class Descriptor>
NLD_AD_DirichletDynamics3D<T,Descriptor>::NLD_AD_DirichletDynamics3D(HierarchicUnserializer& unserializer)
    : NonLocalBoundaryDynamics3D<T,Descriptor>(0)
{
    this->unserialize(unserializer);
}
...
template<typename T, template<typename U> class Descriptor>
NLD_AD_DirichletDynamics3D<T,Descriptor>*
    NLD_AD_DirichletDynamics3D<T,Descriptor>::clone() const
{
            ...
            BlockLattice3D<T,Descriptor>& lattice )
{
    ...
    Array<T,3> jEq;
    T dummyThetaBar;
    planeComputeMacroscopic(direction, orientation, iX, iY, iZ, lattice, rhoBar, jEq, dummyThetaBar);

    for (int iPop=0; iPop<Descriptor<T>::q; ++iPop) {
        if (Descriptor<T>::c[iPop][direction]==-orientation) {
            plint opp = indexTemplates::opposite<Descriptor<T> >(iPop);
            cell[iPop] =
                dynTempl::bgk_ma1_equilibrium(iPop, rhoBar, jEq) +
            ...
            BlockLattice3D<T,Descriptor>& lattice )
{
    ...
    Array<T,3> jEq;
    T dummyThetaBar;
    edgeComputeMacroscopic(plane, normal1, normal2, iX, iY, iZ, lattice, rhoBar, jEq, dummyThetaBar);
            ...
            BlockLattice3D<T,Descriptor>& lattice )
{
    ...
    Array<T,3> jEq;
    T dummyThetaBar;
    cornerComputeMacroscopic(xNormal,yNormal,zNormal, iX, iY, iZ, lattice, rhoBar, jEq, dummyThetaBar);
            ...
            T& rhoBar, Array<T,3>& j, T& thetaBar )
{
    ...
    if (!(dyn1 || dyn2)) {
        ...
        if (same1 && !same2) {
            ...
            this->staticPlaneComputeMacroscopic(direction1, normal1, nbCell2[0], nbCell2[1], nbCell2[2],
                                          lattice, rhoBar, j, thetaBar);
            return;
        }
        // Neighbor2 is Neumann, while neighbor1 is something else --> the outlet normal goes like normal1.
        if (same2 && !same1) {
            // Extrapolate the density from neighbor1, which is on a plane with normal normal2.
            this->staticPlaneComputeMacroscopic(direction2, normal2, nbCell1[0], nbCell1[1], nbCell1[2],
                                          lattice, rhoBar, j, thetaBar);
            return;
        }
        // Either both or none of them is Neumann. In this case, there is no preferred direction,
            ...
            T& rhoBar, Array<T,3>& j, T& thetaBar )
{
    ...
    if (!(dyn1 || dyn2 || dyn3)) {
        ...
        if (same1 && same2 && !same3) {
            ...
            this->staticEdgeComputeMacroscopic(2, xNormal, yNormal, nbCell3[0], nbCell3[1], nbCell3[2],
                                         lattice, rhoBar, j, thetaBar);
            return;
        }
        // Neighbor 1 and 3 are Neumann, while neighbor 2 is something else --> the outlet normal goes like normal2.
        if (same1 && same3 && !same2) {
            ...
            this->staticEdgeComputeMacroscopic(1, zNormal, xNormal, nbCell2[0], nbCell2[1], nbCell2[2],
                                         lattice, rhoBar, j, thetaBar);
            return;
        }
        // Neighbor 2 and 3 are Neumann, while neighbor 1 is something else --> the outlet normal goes like normal1.
        if (same2 && same3 && !same1) {
            ...
            this->staticEdgeComputeMacroscopic(0, yNormal, zNormal, nbCell1[0], nbCell1[1], nbCell1[2],
                                         lattice, rhoBar, j, thetaBar);
            return;
        }
