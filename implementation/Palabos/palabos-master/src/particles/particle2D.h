...
public:
    ...
    Array<T,2> const& getPosition() const { return position; }
    Array<T,2>& getPosition() { return position; }
...
public:
    ...
    virtual bool setVectors(std::vector<Array<T,2> > const& vectors);
    virtual void rescale(int dxScale, int dtScale);
    Array<T,2> const& getVelocity() const { return velocity; }
...
public:
    ...
    NormedVelocityParticle2D(plint tag_, Array<T,2> const& position_, Array<T,2> const& velocity_,
...
public:
    ...
    Array<T,2> const& get_v() const { return v; }
    Array<T,2> const& get_vHalfTime() const { return vHalfTime; }
    Array<T,2> const& get_a() const { return a; }
    T getFluidCompliance() const { return fluidCompliance; }
    T get_rho() const { return rho; }
    T get_invRho() const { return invRho; }

    void set_v(Array<T,2> const& v_) { v = v_; }
    void set_vHalfTime(Array<T,2> const& vHalfTime_) { vHalfTime = vHalfTime_; }
    void set_a(Array<T,2> const& a_) { a = a_; }
...
public:
    ...
    VerletGeryaParticle2D(plint tag_, Array<T,2> const& position_,
                             T fluidConcentration_);
    //virtual void advance();
    virtual void serialize(HierarchicSerializer& serializer) const;
    virtual void unserialize(HierarchicUnserializer& unserializer);
