        ...
        std::vector<Dot3D>& cellPos, std::vector<T>& weights )
{
    ...
    cellPos[3] = Dot3D( (plint) position[0],         (plint)(position[1]+(T)1.0), (plint)(position[2]+(T)1.0) );
    cellPos[4] = Dot3D( (plint)(position[0]+(T)1.0), (plint) position[1],         (plint) position[2] );
    cellPos[5] = Dot3D( (plint)(position[0]+(T)1.0), (plint) position[1],         (plint)(position[2]+(T)1.0) );
    cellPos[6] = Dot3D( (plint)(position[0]+(T)1.0), (plint)(position[1]+(T)1.0), (plint) position[2] );
    cellPos[7] = Dot3D( (plint)(position[0]+(T)1.0), (plint)(position[1]+(T)1.0), (plint)(position[2]+(T)1.0) );
        ...
        std::vector<T>& result)
{
    ...
    for (plint i=0; i<n; ++i) {
        for (plint iCell=0; iCell<8; ++iCell) {
            result[i] += weights[iCell]*
                             tensorField.get(pos[iCell].x,pos[iCell].y,pos[iCell].z)[i];
        }
    }
}
        ...
        TensorField3D<T,nDim>& tensorField, Array<T,3> const& position, T scaling )
{
    ...
    vector1.resetToZero();
    for (plint iCell=0; iCell<8; ++iCell) {
        vector1 += weights[iCell]*tensorField.get(pos[iCell].x,pos[iCell].y,pos[iCell].z)*scaling;
    }
    ...
    vector2.resetToZero();
    for (plint iCell=0; iCell<8; ++iCell) {
        vector2 += weights[iCell]*tensorField.get(pos[iCell].x,pos[iCell].y,pos[iCell].z)*scaling;
    }
