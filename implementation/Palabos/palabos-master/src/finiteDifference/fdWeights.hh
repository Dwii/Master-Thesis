...
template<typename T, int order, int maxWidth>
FdWeights<T, order, maxWidth>& FdWeights<T, order, maxWidth>::operator=(FdWeights<T, order, maxWidth> const& rhs)
{
...
#define PLB_IND(i, j) ((i) * mp1 + (j))
    ...
    for (int i = 1; i <= n; i++) {
        ...
        for (int j = 0; j <= i - 1; j++) {
            ...
            if (j == i - 1) {
                for (int k = mn; k >= 1; k--) {
                    c[PLB_IND(i, k)] = c1 * ((T) k * c[PLB_IND(i - 1, k - 1)] - c5 * c[PLB_IND(i - 1, k)]) / c2;
                }
