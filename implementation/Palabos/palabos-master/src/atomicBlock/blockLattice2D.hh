        ...
        AtomicBlock2D const& from, modif::ModifT kind )
{
    ...
    switch(kind) {
        case modif::staticVariables:
            attribute_static(toDomain, deltaX, deltaY, fromLattice); break;
        case modif::dynamicVariables:
            attribute_dynamic(toDomain, deltaX, deltaY, fromLattice); break;
        case modif::allVariables:
            attribute_all(toDomain, deltaX, deltaY, fromLattice); break;
        case modif::dataStructure:
            attribute_regenerate(toDomain, deltaX, deltaY, fromLattice); break;
        default:
            PLB_ASSERT( false );
    }
}
        ...
        BlockLattice2D<T,Descriptor> const& from )
{
    ...
    for (plint iX=toDomain.x0; iX<=toDomain.x1; ++iX) {
        for (plint iY=toDomain.y0; iY<=toDomain.y1; ++iY) {
            ...
            serialize (
                from.get(iX+deltaX,iY+deltaY).getDynamics(),
                serializedData );
            unserialize (
