...
for line in open(benchmark,'r'):
    ...
    domains += ( r"{0}$^3$".format(nx), ) #+= ( "{0}x{1}x{2}".format(nx, ny, nz), )
    nb_setups += 1
...
for i, argi in enumerate(range(FIRST_ARGS, len(sys.argv), 2)):
    ...
    time[i] = (lat * iterations) / dats_mlups[i] / 1000000
...
for i, mlups in reversed(list(enumerate(dats_mlups))):
    height = time[i] if i != 0 else time[i]-bottom
    height = [max(0, h) for h in height]
...
lgd =plt.legend(bars, legends, loc='upper left')#, bbox_to_anchor=(0.5,-0.1))
